...
$.link_search=function(a,b){""!=b&&(b="foreignid"==a?a+"("+$.trim(b).trim("'")+")":"id"==a||"ppnlink"==a?"id"!=a&&"year"!=a?a+"("+b+")":a+"("+b.replace(/[^a-z0-9]/gi,"")+")":"id"!=a&&"year"!=a?a+"('"+b+"')":a+"("+b.replace(/[^a-z0-9]/gi,"")+")",$.close_dialogs(),$.new_search("link",b))};$.navbar_search=function(a){a=$(a).val();$.new_search("init",a)};
...
success:function(a){var e="";$.each(a,function(a,b){e+="<button onclick='$.openLink(\""+b+"\")' class='"+linkresolverclass+" btn btn-default btn-exemplar'>"+configlang.FULLTEXT+" ("+configlang[a.toUpperCase()]+")</button>"});""!=e?0<$("#linkresolvercontainer_"+b).length?$("#linkresolvercontainer_"+b).html("<div>"+configlang.ACCESS+"</div><div class='container-fluid'><div class='row'>"+e+"</div></div>"):$("#linkresolver_"+b).html(e):($("#linkresolvercontainer_"+b).html(""),$("#linkresolver_"+b).html(""));
...
$.shelve=function(a,b,c,e){c=a="";for(var d in e)"action"!=d&&"typ"!=d&&"id"!=d&&""!=e[d]&&(""==c&&(c=d.replace(/\d/g,"")),""!=c&&c!=d.replace(/\d/g,"")&&(c=d.replace(/\d/g,""),a+="<br />"),a+=e[d].replace(/%lt%/g,"&lt;").replace(/%gt%/g,"&gt;"),"list"!=c&&(a+="<br />"));a+='<br /><div id="shelve_messagebar"></div>';e=$("#"+b+" div.panel-heading").html();void 0==e&&(e=$("#related_"+b).html());var h=new BootstrapDialog({message:a,title:e,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+
configlang.CANCEL,action:function(a){a.close();$.focus_first()}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(){dialogs.push(h)}});h.realize();h.getModalHeader().addClass("preview-header");h.setData("name","shelve");h.open()};
$.reservation=function(a,b,c,e){if(void 0!=e.id){var d=c="";for(f in e)if("action"!=f&&"typ"!=f&&"id"!=f&&"iln"!=f&&""!=e[f])if(""==d&&(d=f.replace(/\d/g,"")),""!=d&&d!=f.replace(/\d/g,"")&&("radio"!=d&&(c+="<br />"),d=f.replace(/\d/g,"")),"radio"==d)if("radio0"==f)c+=e[f]+"<br />";else{var h=e[f].split("|");c+="<div class='radio'><label><input type='radio' value='"+h[0]+"' name='counter'>"+h[1]+"</label></div>"}else"hidden"==d?c+="<input class='hidden' type='text' value='"+e[f]+"' name='counter'>":
