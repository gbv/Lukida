var Scroll=!1,Height=$(document).height(),title="",user,pw,status,username,mailfrom,mailto,msg,config,usrconfig="",configlang,ilconfig="",xtitle=document.title,slider1,slider2,slider3,dialogs=[],waitdialog,kxptimeout=15E3,messid=0,appstate={first:!0},querystate=!1,results={hits:0,hitspack:0,hitscurr:0,qtime:"",ppnlist:{},spell:{},status:0},searchvals={text:"",last:"",package:1,inittext:"",initfacets:{}},facetvals={datapool:"",iln:"",phonetic:!1,yearrange:"",yearmin:"",yearmax:"",yearminstart:"",yearmaxstart:"",
yearminbase:"",yearmaxbase:"",typ:"total",formats:{},sort:"scoredesc"},statevals={ppnlist:[],language:"",layout:""},favors={},favormails={},favorprint={},idleTimer,time2warn=1080,time2kill=120,timer=!1,maxrenewals=0,urlport=location.protocol+"//",linkresolver=!1,linkresolverclass="col-xs-12 col-sm-6 col-md-4",delay=function(){var a=0;return function(b,c){clearTimeout(a);a=setTimeout(b,c)}}(),marcCheck=!0,formate={Article:1,Book:2,Journal:4,Serial_Volume:8,Sound_Recording:16,Musical_Score:32,Microform:64,
Map:128,Motion_Picture:256,Monograph_Series:512,Manuscript:1024,unknown:2048,Game:4096,Data_Media:8192,Projected_Medium:16384,Mixed_Materials:32768,Picture:65536},formats={Article:!1,Book:!1,Journal:!1,"Serial Volume":!1,"Sound Recording":!1,"Musical Score":!1,Microform:!1,Map:!1,"Motion Picture":!1,"Monograph Series":!1,Manuscript:!1,unknown:!1,Game:!1,"Data Media":!1,"Projected Medium":!1,"Mixed Materials":!1,Picture:!1},formatslanguagesort=[],view={},filters=[],settings={},settingsname,isMobile=
window.matchMedia("only screen and (max-width: 760px)").matches,intervalID="",$JSONData,answer={status:"",message:""},resolver={fullview:null,google:null,link:null,simular:null,export:null,imgurl:null},SettingsTooltip=!1;$.is_Empty=function(a){if(null==a)return!0;if(0<a.length)return!1;if(0===a.length||"object"!==typeof a)return!0;for(var b in a)if(hasOwnProperty.call(a,b))return!1;return!0};$.focus_first=function(){};
...
$.openLink=function(a){$.post("/ajax/statsclient",{typ:"Link",name:a,range:"year"});window.open(a,"_blank")};$.toggle_area=function(a){$(":focus").blur();$("."+a+"up").toggleClass("collapse");$("."+a+"down").toggleClass("collapse");view.hasOwnProperty(a)||(view[a]=!1);$(".discover"+a).hasClass("collapse")?$(".discover"+a).slideDown("fast").removeClass("collapse"):$(".discover"+a).slideUp("fast").addClass("collapse");view[a]=!view[a]};$.close_dialogs=function(){for(var a=dialogs.length-1;0<=a;a--)dialogs[a].close()};
...
$.clear_search_after_timeout=function(){$.close_dialogs();$("#facets").fadeOut("fast").addClass("collapse");$("#results").addClass("collapse");$("#resultsonly").html("");$("#loading").fadeOut("fast").addClass("collapse");$("#start").fadeIn("fast").removeClass("collapse");$("#version_start").fadeIn("fast").removeClass("collapse");$("#version_search").fadeOut("fast").addClass("collapse");results={hits:0,hitspack:0,hitscurr:0,qtime:"",ppnlist:{},spell:{},status:0};searchvals={text:"",last:"",package:1,
inittext:"",initfacets:{}};facetvals.yearrange="";facetvals.yearmin="";facetvals.yearmax="";facetvals.yearminstart="";facetvals.yearmaxstart="";facetvals.typ="total";facetvals.formats={};facetvals.sort="scoredesc";$.replace_state();$.focus_first()};$.cleanSearch=function(a){if(null===a||""===a)return"*";a=a.toString();a=a.replace(/(<([^>]+)>)/ig,"");a=a.trim().replace(/,/g,", ").replace(/  /g," ");var b=document.createElement("div");b.innerHTML=a;return b.textContent||b.innerText||"*"};
$.new_search=function(a,b){if("facet_"!=a.substr(0,6).toLowerCase()&&"onpop"!=a){searchvals.last=searchvals.text;var c=$.cleanSearch(b);b=c;searchvals.text=c;if("cmd:"==searchvals.text.substr(0,4).toLowerCase()){$.command(searchvals.text.substr(4));$("#searchtext_xs").val(searchvals.last);$("#searchtext_md").val(searchvals.last);searchvals.text=searchvals.last;$.focus_first();return}$("#searchtext_xs").val(b);$("#searchtext_md").val(b);facetvals.yearrange="[1000 TO "+((new Date).getFullYear()+1)+
"]";facetvals.yearmin=1E3;facetvals.yearmax=(new Date).getFullYear()+1;facetvals.yearminstart=1E3;facetvals.yearmaxstart=(new Date).getFullYear()+1;facetvals.typ="total";facetvals.formats={};facetvals.sort="scoredesc"}Scroll=!1;searchvals.package=1;searchvals.search=searchvals.text;$.open_wait();$.search_results(a,function(){$.close_wait();void 0!==results.status&&-2==results.status?($.clear_search_after_timeout(),$.user_message(configlang.GENERALPROBLEM,"warning",15E3)):void 0!==results.status&&
-1==results.status?($.clear_search_after_timeout(),$.user_message(configlang.FINDEXTIMEOUT,"warning",15E3)):0==results.hits&&$.spell_suggestion();$.push_state();$.focus_first()})};
$.search_results=function(a,b){$.ajax({url:"/ajax/search/",timeout:kxptimeout,method:"POST",data:{search:searchvals.search,package:searchvals.package,facets:JSON.stringify(facetvals)},dataType:"JSON",success:function(c){$JSONData=c;$JSONData.hasOwnProperty("status")&&0==$JSONData.status?(results.status=$JSONData.status,results.hits=$JSONData.hits,results.hitspack=$JSONData.hitspack,results.qtime=$JSONData.qtime,results.spell=$JSONData.spell,c=$.trim($JSONData.query),c+=" <a href='"+c+"' target='_blank'><span class='fa fa-external-link'></span></a>",
...
$("#btn_facets").removeClass("hidden-xs"))),Scroll=!1,50==results.hitspack&&(Scroll=!0)):results.status=$JSONData.status;b()},error:function(c,d){results.status="timeout"===d?-1:-2;b()}})};$.search_next_pack=function(a){Scroll=!1;$("#loading").removeClass("hideLoading").addClass("displayLoading");searchvals.package++;$.search_results("adddata",function(){$("#loading").removeClass("displayLoading").addClass("hideLoading");a()})};
...
$.link_search=function(a,b){""!=b&&("foreignid"==a?b=a+"("+$.trim(b).trim("'")+")":"id"==a||"ppnlink"==a||"norm"==a?b=a+"("+b+")":"year"==a?b=a+"("+b.replace(/[^a-z0-9]/gi,"")+")":"classlocal"==a?b=a+"("+b.replace(/[^a-z0-9. ]/gi,"")+")":"text"!=a&&(b=a+"('"+b+"')"),$.close_dialogs(),$.new_search("link",b))};$.navbar_search=function(a){a=$(a).val();$.new_search("init",a)};
...
urlport+window.location.host+"/"+a+'"><b>'+a+"</b></a> <span class='fa fa-link'></span>"),"success",15E3)};
$.close_resolvers=function(){null!=resolver.fullview&&(resolver.fullview.abort(),resolver.fullview=null);null!=resolver.google&&(resolver.google.abort(),resolver.google=null);null!=resolver.link&&(resolver.link.abort(),resolver.link=null);null!=resolver.simular&&(resolver.simular.abort(),resolver.simular=null);null!=resolver.export&&(resolver.export.abort(),resolver.export=null);null!=resolver.imgurl&&(resolver.imgurl.abort(),resolver.imgurl=null)};
...
$.googleresolver=function(a,b){if(config.googlecover||config.googlepreview){var c=$("#isbn_"+a).data("isbn");""!=c&&void 0!=c?resolver.google=$.ajax({url:"https://books.google.com/books?bibkeys=ISBN%3A"+c+"&jscmd=viewapi&callback=listEntries",method:"POST",crossDomain:!0,dataType:"jsonp",success:function(d){$.each(d,function(e,h){h.hasOwnProperty("embeddable")&&h.hasOwnProperty("thumbnail_url")&&h.hasOwnProperty("info_url")&&$("#isbn_"+a).html("<a target='_blank' href='"+h.info_url+"'><img class='img-responsive' src='"+
h.thumbnail_url+"' title='"+configlang.GOOGLEBOOKS+"' data-tooltip='tooltip' data-placement='buttom'></a>");h.hasOwnProperty("preview")&&"partial"==h.preview&&h.hasOwnProperty("preview_url")&&$("#isbn_"+a).append("<p></p><a target='_blank' href='"+h.preview_url+"'><img src='systemassets/lukida/img/google_preview_"+config.language+".gif'></a>")});$("#isbn_"+a).val(d);b()}}):b()}else b()};
...
success:function(d){var e="";$.each(d,function(h,l){e+="<button onclick='$.openLink(\""+l+"\")' class='"+linkresolverclass+" btn btn-default btn-exemplar'>"+configlang.FULLTEXT+"</button>"});""!=e?0<$("#linkresolvercontainer_"+b).length?$("#linkresolvercontainer_"+b).html("<div>"+configlang.ACCESS+"</div><div class='container-fluid'><div class='row'>"+e+"</div></div>"):$("#linkresolver_"+b).html(e):($("#linkresolvercontainer_"+b).html(""),$("#linkresolver_"+b).html(""));c()}})):($("#linkresolvercontainer_"+
b).html(""),$("#linkresolver_"+b).html(""),c())};$.simularpublications=function(a,b,c){config.simularpubs?resolver.simular=$.ajax({url:"/ajax/searchsimularpubs/",method:"POST",data:{ppn:a},dataType:"JSON",success:function(d){0<d.hitspack?$("#simularpubscontent_"+b).html(d.results):$("#simularpubscontent_"+b).html("<div class='col-xs-12'><div class='well'>"+configlang.NOSIMULARHITS+"</div></div>");c()}}):c()};
...
$.create_facets_formats=function(){var a="",b="",c="",d="",e="",h=Math.ceil(formatslanguagesort.length/2);$.each(formatslanguagesort,function(l,f){valcount=void 0!==$JSONData.formats[f.id]?$JSONData.formats[f.id]:"";b=f.val;a="<div class='checkbox'><label><input class='facetformat' id='"+f.id+"' value='"+f.id+"' type='checkbox'><span class='lbl_"+f.id.replace(/ /g,"").toUpperCase()+"'>"+b+"</span></label><span class='num_"+f.id.replace(/ /g,"").toUpperCase()+" numformat badge counter pull-right'>"+
valcount+"</span></div>";c+=a;b=12<f.val.length?f.val.substring(0,12)+".":f.val;a="<div class='checkbox'><label><input class='facetformat' id='"+f.id+"' value='"+f.id+"' type='checkbox'><span class='lbl_"+f.id.replace(/ /g,"").toUpperCase()+"'>"+b+"</span></label><span class='num_"+f.id.replace(/ /g,"").toUpperCase()+" numformat badge counter pull-right'>"+valcount+"</span></div>";l<h?d+=a:e+=a});$("#formats1").html(c);$("#formats2").html("<div class='row'><div class='col-xs-6'>"+d+"</div><div class='col-xs-6'>"+
e+"</div></div>");$.each(facetvals.formats,function(l,f){$("#formats1").find("input[value='"+l+"']").prop("checked",!0);$("#formats2").find("input[value='"+l+"']").prop("checked",!0)});$(".facetformat").change(function(){"formats1"==$(this).closest(".formats").attr("id")&&$("#formats2").find("input[value='"+$(this).val()+"']").prop("checked",this.checked);"formats2"==$(this).closest(".formats").attr("id")&&$("#formats1").find("input[value='"+$(this).val()+"']").prop("checked",this.checked);$.set_facet("format",
...
$.open_fullview=function(a,b,c,d){void 0===b&&(b=results.ppnlist);void 0===d&&(d=a);void 0===c&&(c="main");"main"!=c&&(b=jQuery.map(b,function(g,r){return g.toString()}));var e=dialogs.length,h=a in favors?configlang.CHECKLISTREMOVE:configlang.CHECKLIST,l=a in favors?"fa fa-star-o":"fa fa-star",f="publications"==c?$("#related_"+d).html():$("#"+d+" div.panel-heading").html(),k=new BootstrapDialog({message:'<div id="fullbody_'+e+'"><div class="outercircle"></div><div class="innercircle"></div></div>',
title:'<div id="fullheader_'+e+'">'+f+"</div>",size:BootstrapDialog.SIZE_WIDE,closable:!0,buttons:[{id:"prevbtn",icon:"glyphicon glyphicon-chevron-left",cssClass:"btn-default fullview-button-left hidden-xs hidden-sm",action:function(g){$(this).blur();0<b.indexOf(a)?(a=b[b.indexOf(a)-1],$("#fullbody_"+e).slideUp("fast","swing",function(){$.set_fullviewppn(a,e,c,function(){$("#fullbody_"+e).slideDown("fast")})})):$.user_message(configlang.FIRSTOBJECT,"warning")}},{id:"nextbtn",icon:"glyphicon glyphicon-chevron-right",
cssClass:"btn-default fullview-button-left hidden-xs hidden-sm",action:function(g){$(this).blur();"main"==c&&b.indexOf(a)<results.hits-1&&b.indexOf(a)==results.hitscurr-1?$.search_next_pack(function(){b=results.ppnlist;a=b[b.indexOf(a)+1];$("#fullbody_"+e).slideUp("fast","swing",function(){$.set_fullviewppn(a,e,c,function(){$("#fullbody_"+e).slideDown("fast")})})}):b.indexOf(a)<b.length-1?(a=b[b.indexOf(a)+1],$("#fullbody_"+e).slideUp("fast","swing",function(){$.set_fullviewppn(a,e,c,function(){$("#fullbody_"+
e).slideDown("fast")})})):$.user_message(configlang.LASTOBJECT,"warning")}},{label:" "+configlang.CANCEL,icon:"glyphicon glyphicon-menu-up",cssClass:"btn-default hidden-xs hidden-sm",action:function(g){g.close();$.focus_first()}},{id:"qrcodebtn_"+e,label:" "+configlang.QRCODE,icon:"fa fa-qrcode",cssClass:"btn-default hidden-xs hidden-sm",action:function(g){$(this).blur();$.open_qrcode()}},{id:"printbtn_"+e,label:" "+configlang.PRINT,icon:"fa fa-print",cssClass:"btn-default hidden-xs hidden-sm",action:function(g){$(this).blur();
g=window.open();g.document.write("<html><head><title>"+xtitle+"</title></head><body><p><img src='"+config.printlogo+"' style='max-width:300px;' />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span><b>"+config.softwarename+" - "+urlport+window.location.host+"</b></span></p>");g.document.write("<table>"+$.getfullbody()+"</table>");g.document.write("</body></html");g.document.close();g.focus();g.print();g.close()}},{id:"mailbtn_"+e,label:" "+configlang.MAIL,icon:"glyphicon glyphicon-envelope",cssClass:"btn-default hidden-xs hidden-sm",
action:function(g){$(this).blur();favormails[a]="<table>"+$.getfullbody()+"</table>";""==usrconfig?$.login("LOGINFIRST",function(){""!=usrconfig&&$.open_mail([a])}):$.open_mail([a])}},{id:"exportbtn_"+e,label:" "+configlang.EXPORT,icon:"glyphicon glyphicon-export",cssClass:"btn-default hidden-xs hidden-sm",action:function(g){$(this).blur();$.open_export([a])}},{id:"checklistbtn_"+e,label:' <span class="text">'+h+"</span>",icon:l,cssClass:"btn-default checklist hidden-xs hidden-sm",action:function(g){$(this).blur();
a in favors?(delete favors[a],delete favormails[a],delete favorprint[a],$.user_message(configlang.CHECKLISTREMOVESUCCESS,"success"),g='<span class="bootstrap-dialog-button-icon fa fa-star"></span> <span class="text">'+configlang.CHECKLIST+"</span>"):(favors[a]=$("#"+a).clone().removeClass("medium col-xs-12 col-sm-6 col-md-4 col-lg-3","col-md-6").addClass("merk col-md-6"),$(favors[a]).find("a").attr("href","javascript:$.open_fullview('"+a+"',Object.keys(favors));"),favormails[a]="<table>"+$.getfullbody()+
"</table>",favorprint[a]="<table>"+$.getfullbody()+"</table>",$.user_message(configlang.CHECKLISTSUCCESS,"success"),g='<span class="bootstrap-dialog-button-icon fa fa-star-o"></span> <span class="text">'+configlang.CHECKLISTREMOVE+"</span>");$("#checklistbtn_"+e).html(g);$("#checklistbtn_xs_"+e).html(g);$.update_favors()}},{id:"prevbtn_xs_"+e,icon:"glyphicon glyphicon-chevron-left",cssClass:"btn-default fullview-button-left hidden-md hidden-lg col-xs-2 col-sm-1 NoLeftMargin",action:function(g){$(this).blur();
0<b.indexOf(a)?(a=b[b.indexOf(a)-1],$("#fullbody_"+e).slideUp("fast","swing",function(){$.set_fullviewppn(a,e,c,function(){$("#fullbody_"+e).slideDown("fast")})})):$.user_message(configlang.FIRSTOBJECT,"warning")}},{id:"nextbtn_xs_"+e,icon:"glyphicon glyphicon-chevron-right",cssClass:"btn-default fullview-button-left hidden-md hidden-lg col-xs-2 col-sm-1 NoLeftMargin",action:function(g){$(this).blur();"main"==c&&b.indexOf(a)<results.hits-1&&b.indexOf(a)==results.hitscurr-1?$.search_next_pack(function(){b=
results.ppnlist;a=b[b.indexOf(a)+1];$("#fullbody_"+e).slideUp("fast","swing",function(){$.set_fullviewppn(a,e,c,function(){$("#fullbody_"+e).slideDown("fast")})})}):b.indexOf(a)<b.length-1?(a=b[b.indexOf(a)+1],$("#fullbody_"+e).slideUp("fast","swing",function(){$.set_fullviewppn(a,e,c,function(){$("#fullbody_"+e).slideDown("fast")})})):$.user_message(configlang.LASTOBJECT,"warning")}},{id:"checklistbtn_xs_"+e,label:' <span class="text">'+h+"</span>",icon:l,cssClass:"btn-default checklist hidden-md hidden-lg col-xs-4 col-sm-4 LeftOffset2",
action:function(g){$(this).blur();a in favors?(delete favors[a],delete favormails[a],delete favorprint[a],$.user_message(configlang.CHECKLISTREMOVESUCCESS,"success"),g='<span class="bootstrap-dialog-button-icon fa fa-star"></span> <span class="text">'+configlang.CHECKLIST+"</span>"):(favors[a]=$("#"+a).clone().removeClass("medium col-xs-12 col-sm-6 col-md-4 col-lg-3","col-md-6").addClass("merk col-md-6"),$(favors[a]).find("a").attr("href","javascript:$.open_fullview('"+a+"',Object.keys(favors));"),
favormails[a]="<table>"+$.getfullbody()+"</table>",favorprint[a]="<table>"+$.getfullbody()+"</table>",$.user_message(configlang.CHECKLISTSUCCESS,"success"),g='<span class="bootstrap-dialog-button-icon fa fa-star-o"></span> <span class="text">'+configlang.CHECKLISTREMOVE+"</span>");$("#checklistbtn_"+e).html(g);$("#checklistbtn_xs_"+e).html(g);$.update_favors()}},{label:" "+configlang.CANCEL,icon:"glyphicon glyphicon-menu-up",cssClass:"btn-default hidden-md hidden-lg col-xs-4 col-sm-2 LeftOffset2",
action:function(g){g.close();$.focus_first()}},{id:"qrcodebtn_xs_"+e,label:" QR",icon:"fa fa-qrcode",cssClass:"btn-default hidden-md hidden-lg col-xs-2 col-sm-2 NoLeftMargin",action:function(g){$(this).blur();$.open_qrcode()}},{id:"exportbtn_xs_"+e,label:" "+configlang.EXPORT,icon:"glyphicon glyphicon-export",cssClass:"btn-default hidden-md hidden-lg col-xs-4 col-sm-2 LeftOffset2",action:function(g){$(this).blur();$.open_export([a])}},{id:"printbtn_xs_"+e,label:" "+configlang.PRINT,icon:"fa fa-print",
cssClass:"btn-default hidden-md hidden-lg col-xs-3 col-sm-2 NoLeftMargin",action:function(g){$(this).blur();g=window.open();g.document.write("<html><head><title>"+xtitle+"</title></head><body><p><img src='"+config.printlogo+"' style='max-width:300px;' />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span><b>"+config.softwarename+" - "+urlport+window.location.host+"</b></span></p>");g.document.write("<table>"+$.getfullbody()+"</table>");g.document.write("</body></html");g.document.close();g.focus();
g.print();g.close()}},{id:"mailbtn_xs_"+e,label:" "+configlang.MAIL,icon:"glyphicon glyphicon-envelope",cssClass:"btn-default hidden-md hidden-lg col-xs-3 col-sm-2 LeftOffset2",action:function(g){$(this).blur();favormails[a]="<table>"+$.getfullbody()+"</table>";""==usrconfig?$.login("LOGINFIRST",function(){""!=usrconfig&&$.open_mail([a])}):$.open_mail([a])}}],onhidden:function(){dialogs.pop();$.show_answer();statevals.ppnlist.pop();$.replace_state()},onshown:function(){dialogs.push(k);$.set_fullviewppn(a,
e,c,function(){$.is_Empty(statevals)&&(statevals={});$.is_Empty(statevals.ppnlist)&&(statevals.ppnlist=[]);statevals.ppnlist.push({ppn:a,ppnlist:b,typ:c,parentppn:d})})}});k.realize();k.getModalHeader().addClass("preview-header");k.setData("ppn",a);k.setData("name","fullview_"+e);k.open()};
...
$.open_settings=function(){var a=dialogs.length;JSON.stringify(settings);var b=facetvals.datapool,c=facetvals.phonetic,d=new BootstrapDialog({message:$('<div id="settings_'+a+'"></div>').load("/ajax/settings/"+a,function(){}),title:configlang.SETTINGS,size:BootstrapDialog.SIZE_WIDE,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(e){e.close();$.focus_first()}},{icon:"fa fa-check",label:" OK",cssClass:"fullview-button-color",action:function(e){$(this).blur();
$.update_settings(function(){facetvals.typ="total";facetvals.formats={};facetvals.yearmin=facetvals.yearminbase;facetvals.yearmax=facetvals.yearmaxbase;facetvals.yearminstart=facetvals.yearminbase;facetvals.yearmaxstart=facetvals.yearmaxbase;facetvals.yearrange="["+facetvals.yearminbase+" TO "+facetvals.yearmaxbase+"]";var h=$("#searchtext_md").val();b==facetvals.datapool&&c==facetvals.phonetic||!h||$.new_search("init",$("#searchtext_md").val());e.close()})}}],onhidden:function(){dialogs.pop();$.show_answer()},
onshown:function(){"global"==facetvals.datapool&&$("#communitylibraries").prop("checked",!0);1==facetvals.phonetic&&$("#phoneticSwitch").prop("checked",!0);dialogs.push(d)}});d.realize();d.getModalHeader().addClass("preview-header");d.setData("name","settings");d.open()};
...
$.open_user=function(a){var b=dialogs.length,c=new BootstrapDialog({message:'<div id="userbody_'+b+'"><div class="outercircle"></div><div class="innercircle"></div></div>',title:configlang.AREA+" "+usrconfig.firstname+" "+usrconfig.lastname,size:BootstrapDialog.SIZE_WIDE,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(d){d.close();$.focus_first()}},{icon:"glyphicon glyphicon-triangle-bottom",label:" "+configlang.OK,cssClass:"fullview-button-color",
action:function(d){d.close();$.focus_first()}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(){dialogs.push(c);$.set_userview(a,b,function(){})}});c.realize();c.getModalHeader().addClass("preview-header");c.setData("name","user");c.open()};$.set_userview=function(a,b,c){$.close_resolvers();resolver.userview=$.ajax({url:"/ajax/userview/"+a,method:"GET",crossDomain:!0,success:function(d){$("#userbody_"+b).html(d);c()}})};
...
$.login=function(a,b){var c="",d="",e="",h="",l=new BootstrapDialog({message:'<div class="form-horizontal"><div class="form-group"><label for="kennung" class="col-sm-3 control-label">'+configlang.USERNUMBER+'</label><div class="col-sm-9"><input type="text" class="form-control keyenter" id="kennung" placeholder="'+configlang.USERNUMBER+'" style="text-transform:uppercase" style="text-transform:uppercase"></div></div><div class="form-group"><label for="passwort" class="col-sm-3 control-label">'+configlang.PASSWORD+
'</label><div class="col-sm-9"><input type="password" class="form-control keyenter" id="passwort" placeholder="'+configlang.PASSWORD+'"></div></div><div id="login_messagebar"></div></div>',title:"Login "+config.softwarename,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(f){f.close();$.focus_first()}},{icon:"glyphicon glyphicon-log-in",label:" "+configlang.LOGIN,cssClass:"fullview-button-color",id:"loginbutton",action:function(f){$(this).blur();
user=$("#kennung").val().trim().toUpperCase();pw=$("#passwort").val().trim();if(""==user||""==pw)return $.user_message(configlang.REQUIREDTWOFIELDS,"warning"),""==user?$("#kennung").focus():$("#passwort").focus(),-1;$.open_wait();$.close_resolvers();$.post("/ajax/login",{user,pw},function(k){$.close_wait();"-1"!=k?(f.close(),$(".nologinarea").addClass("hide"),$(".loginarea").removeClass("hide"),usrconfig=k,timer=!0,$.startTimer(),answer.status="success",answer.message=configlang.LOGINSUCCESS.replace("{firstname}",
k.firstname).replace("{lastname}",k.lastname),k.message&&(c="warning",d=k.messagetext),k.note&&(e="warning",h=k.note),b()):($.user_message(configlang.LOGINERROR,"warning"),$("#kennung").focus(),usrconfig="");$.focus_first()},"json")}}],onhidden:function(){dialogs.pop();$.show_answer();""!=c&&$.user_message(d,c,1E4);""!=e&&$.user_message(h,e,1E4)},onshown:function(){dialogs.push(l);"1"==config.devmode&&($("#kennung").val(config.devuser),$("#passwort").val(config.devpassword));$("#kennung").focus();
$(".keyenter").keypress(function(f){13==f.which&&($(this).blur(),$("#loginbutton").focus().click())});void 0!=a&&""!=a&&$.user_message(configlang[a],"warning")}});l.realize();l.getModalHeader().addClass("preview-header");l.setData("name","login");l.open()};$.loginonly=function(){$.login("",function(){})};
...
$.shelve=function(a,b,c,d){c=a="";for(var e in d)"action"!=e&&"typ"!=e&&"id"!=e&&"iln"!=e&&"class"!=e&&""!=d[e]&&"data"!=e.replace(/\d/g,"")&&(""==c&&(c=e.replace(/\d/g,"")),""!=c&&c!=e.replace(/\d/g,"")&&(c=e.replace(/\d/g,""),a+="<br />"),a+=d[e].replace(/%lt%/g,"&lt;").replace(/%gt%/g,"&gt;"),"list"!=c&&(a+="<br />"));a+='<br /><div id="shelve_messagebar"></div>';d=$("#"+b+" div.panel-heading").html();void 0==d&&(d=$("#related_"+b).html());var h=new BootstrapDialog({message:a,title:d,closable:!0,
buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(l){l.close();$.focus_first()}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(){dialogs.push(h)}});h.realize();h.getModalHeader().addClass("preview-header");h.setData("name","shelve");h.open()};
$.reservation=function(a,b,c,d){if(void 0!=d.id){var e=c="",h=[];for(f in d)if("action"!=f&&"typ"!=f&&"id"!=f&&"iln"!=f&&"class"!=f&&""!=d[f]&&"data"!=f.replace(/\d/g,""))if("usrgrps"==f)h=d[f];else if(""==e&&(e=f.replace(/\d/g,"")),""!=e&&e!=f.replace(/\d/g,"")&&("radio"!=e&&(c+="<br />"),e=f.replace(/\d/g,"")),"radio"==e)if("radio0"==f)c+=d[f]+"<br />";else{var l=d[f].split("|");c+="<div class='radio'><label><input type='radio' value='"+l[0]+"' name='counter'>"+l[1]+"</label></div>"}else"hidden"==
e?c+="<input class='hidden' type='text' value='"+d[f]+"' name='counter'>":(c+=d[f].replace(/%lt%/g,"&lt;").replace(/%gt%/g,"&gt;"),"list"!=e&&(c+="<br />"));c+='<div id="reservation_messagebar"></div>';var f=$("#"+b+" div.panel-heading").html();void 0==f&&(f=$("#related_"+b).html());var k=new BootstrapDialog({message:c,title:f,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(g){g.close();$.focus_first()}},{icon:"fa fa-map-marker",label:" "+configlang.RESERVATION,
cssClass:"fullview-button-color",action:function(g){$(this).blur();var r="";if($("input[name='counter']").length)if($("input[name='counter']").is(":hidden"))r=$("input[name='counter']:hidden").val();else{if(!$("input[name='counter']").is(":checked")){$.user_message(configlang.SELECTDESK,"warning");return}r=$("input[name='counter']:checked").val()}$.request(a,d.id,configlang.RESERVATIONSUCCESS,r,"reserve",h,function(q){q?.status&&"warning"==q.status?($.user_message(q.message,q.status),g.close(),q.message=
"",q.status=""):g.close()})}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(){dialogs.push(k)}});k.realize();k.getModalHeader().addClass("preview-header");k.setData("name","reservation");k.open()}};
$.order=function(a,b,c,d){if(void 0!=d.id){var e=c="",h=[];for(f in d)if("action"!=f&&"typ"!=f&&"id"!=f&&"iln"!=f&&"class"!=f&&""!=d[f]&&"data"!=f.replace(/\d/g,""))if("usrgrps"==f)h=d[f];else if(""==e&&(e=f.replace(/\d/g,"")),""==e&&(e=f.replace(/\d/g,"")),""!=e&&e!=f.replace(/\d/g,"")&&("radio"!=e&&(c+="<br />"),e=f.replace(/\d/g,"")),"radio"==e)if("radio0"==f)c+=d[f]+"<br />";else{var l=d[f].split("|");c+="<div class='radio'><label><input type='radio' value='"+l[0]+"' name='counter'>"+l[1]+"</label></div>"}else"hidden"==
e?c+="<input class='hidden' type='text' value='"+d[f]+"' name='counter'>":(c+=d[f].replace(/%lt%/g,"&lt;").replace(/%gt%/g,"&gt;"),"list"!=e&&(c+="<br />"));c+='<div id="order_messagebar"></div>';var f=$("#"+b+" div.panel-heading").html();void 0==f&&(f=$("#related_"+b).html());var k=new BootstrapDialog({message:c,title:f,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(g){g.close();$.focus_first()}},{icon:"fa fa-map-marker",label:" "+configlang.ORDER,
cssClass:"fullview-button-color",action:function(g){$(this).blur();var r="";if($("input[name='counter']").length)if($("input[name='counter']").is(":hidden"))r=$("input[name='counter']:hidden").val();else{if(!$("input[name='counter']").is(":checked")){$.user_message(configlang.SELECTDESK,"warning");return}r=$("input[name='counter']:checked").val()}$.request(a,d.id,configlang.ORDERSUCCESS,r,"order",h,function(q){"warning"==q.status?($.user_message(q.message,q.status),q.message="",q.status=""):g.close()})}}],
onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(){dialogs.push(k)}});k.realize();k.getModalHeader().addClass("preview-header");k.setData("name","order");k.open()}};
$.request=function(a,b,c,d,e,h,l){$.close_resolvers();""==usrconfig?$.login("LOGINFIRST",function(){h.length&&usrconfig?.type&&!h.includes(parseInt(usrconfig.type))?(answer.status="warning",answer.message=configlang.PROCESSSTOPPED+"<br />"+configlang.NOAUTH,l(answer)):$.post("/ajax/request",{iln:a,uri:b,desk:d,action:e},function(f){0>f.status?(answer.status="warning",answer.message=configlang.PROCESSSTOPPED+"<br />"+f.error):(answer.status="success",answer.message=c);l(answer)},"json")}):h.length&&
usrconfig?.type&&!h.includes(parseInt(usrconfig.type))?(answer.status="warning",answer.message=configlang.PROCESSSTOPPED+"<br />"+configlang.NOAUTH,l(answer)):$.post("/ajax/request",{iln:a,uri:b,desk:d,action:e},function(f){0>f.status?(answer.status="warning",answer.message=configlang.PROCESSSTOPPED+"<br />"+f.error):(answer.status="success",answer.message=c);l(answer)},"json")};
$.getfullbody=function(){var a="",b=dialogs.length;do $("#biblio_"+b).length&&($tmp=$("#biblio_"+b).clone(),$tmp.find("table").removeAttr("id"),$tmp.find("script").remove(),$tmp.find("button").removeAttr("onclick"),$tmp.find("button").removeAttr("class"),$tmp.find("button").each(function(){$(this).replaceWith("<p>"+$(this).html()+"</p>")}),$tmp.find("img").remove(),$tmp.find("span").removeAttr("class"),$tmp.find("span.fa").remove(),$tmp.find("span.bibsee_0down").remove(),$tmp.find("span.bibsee_1down").remove(),
$tmp.find("span.bibsee_2down").remove(),$tmp.find("span.bibsee_3down").remove(),$tmp.find("span.bibsee_0up").remove(),$tmp.find("span.bibsee_1up").remove(),$tmp.find("span.bibsee_2up").remove(),$tmp.find("span.bibsee_3up").remove(),$tmp.find("span.bibinc_0down").remove(),$tmp.find("span.bibinc_1down").remove(),$tmp.find("span.bibinc_2down").remove(),$tmp.find("span.bibinc_3down").remove(),$tmp.find("span.bibinc_0up").remove(),$tmp.find("span.bibinc_1up").remove(),$tmp.find("span.bibinc_2up").remove(),
$tmp.find("span.bibinc_3up").remove(),$tmp.find("span.bibsum_0down").remove(),$tmp.find("span.bibsum_1down").remove(),$tmp.find("span.bibsum_2down").remove(),$tmp.find("span.bibsum_3down").remove(),$tmp.find("span.bibsum_0up").remove(),$tmp.find("span.bibsum_1up").remove(),$tmp.find("span.bibsum_2up").remove(),$tmp.find("span.bibsum_3up").remove(),$tmp.find("span.bibrem_0down").remove(),$tmp.find("span.bibrem_1down").remove(),$tmp.find("span.bibrem_2down").remove(),$tmp.find("span.bibrem_3down").remove(),
$tmp.find("span.bibrem_0up").remove(),$tmp.find("span.bibrem_1up").remove(),$tmp.find("span.bibrem_2up").remove(),$tmp.find("span.bibrem_3up").remove(),$tmp.find("tr.discoverbibsee_0").remove(),$tmp.find("tr.discoverbibsee_1").remove(),$tmp.find("tr.discoverbibsee_2").remove(),$tmp.find("tr.discoverbibsee_3").remove(),$tmp.find("tr.discoverbibinc_0").remove(),$tmp.find("tr.discoverbibinc_1").remove(),$tmp.find("tr.discoverbibinc_2").remove(),$tmp.find("tr.discoverbibinc_3").remove(),$tmp.find("tr.discoverbibsum_0").remove(),
$tmp.find("tr.discoverbibsum_1").remove(),$tmp.find("tr.discoverbibsum_2").remove(),$tmp.find("tr.discoverbibsum_3").remove(),$tmp.find("tr.discoverbibrem_0").remove(),$tmp.find("tr.discoverbibrem_1").remove(),$tmp.find("tr.discoverbibrem_2").remove(),$tmp.find("tr.discoverbibrem_3").remove(),$tmp.find("td").removeAttr("id"),$tmp.find("td").removeAttr("class"),$tmp.find("td").removeAttr("data-toggle"),$tmp.find("td").removeAttr("title"),$tmp.find("small").removeAttr("id"),$tmp.find("i.fa").remove(),
$tmp.find("div.section_1").remove(),$tmp.find("div.section_2").remove(),$tmp.find("div.section_3").remove(),$tmp.find("div.section_4").remove(),$tmp.find("div").removeAttr("class"),$tmp.find("div").removeAttr("id"),$tmp.find("div").removeAttr("data-ppn"),$tmp.find("div").removeAttr("data-isbn"),$tmp.find("table").removeAttr("class"),$tmp.find("a").removeAttr("class"),$tmp.find("a").removeAttr("href"),$tmp.find("a").removeAttr("target"),$tmp.find("a").removeAttr("tabindex"),$tmp.find("a").removeAttr("role"),
$tmp.find("a").removeAttr("class"),$tmp.find("a").removeAttr("title"),$tmp.find("a").removeAttr("data-container"),$tmp.find("a").removeAttr("data-toggle"),$tmp.find("a").removeAttr("data-trigger"),$tmp.find("a").removeAttr("data-placement"),$tmp.find("a").removeAttr("data-title"),$tmp.find("a").removeAttr("data-html"),$tmp.find("a").removeAttr("data-content"),$tmp.find("a").removeAttr("data-original-title"),a=$tmp.html()),b--;while(""==a&&0<=b);return a};
$.mailorder=function(a,b,c,d){a=$("#"+b+" div.panel-heading").html();void 0==a&&(a=$("#related_"+b).html());var e="ORDER",h="MAILORDER",l="0",f="Magazinbestellung";$.close_resolvers();var k=new BootstrapDialog({message:$('<div id="mailorderviewbody"></div>').load("/ajax/mailorderview/"+b+"/"+c,function(g,r,q){e=$(g).wrapAll($("<div>")).parent().find("#mailbutton").attr("value");h=$(g).wrapAll($("<div>")).parent().find("#mailtyp").attr("value");l=$(g).wrapAll($("<div>")).parent().find("#mailprint").attr("value");
f=$(g).wrapAll($("<div>")).parent().find("#mailsubject").attr("value");k.getButton("mailbtn").html("<span class='bootstrap-dialog-button-icon fa fa-map-marker'></span> "+e)}),title:a,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(g){g.close();$.focus_first()}},{id:"mailbtn",icon:"fa fa-map-marker",label:" "+configlang.ORDER,cssClass:"fullview-button-color",action:function(g){$(this).blur();var r=$("input[name=mailto]:checked").val(),q=$("input[name=mailto]:checked").parent().text();
void 0==r&&(r=$("#mailto").attr("value"));void 0==q&&(q=$("#mailto").data("label"));if(void 0==r)return $.user_message("Die MailTo-Adresse konnte nicht ermittelt werden!","warning"),-1;var w=$("input[name=mailfrom]:checked").val();void 0==w&&(w=$("#mailfrom").attr("value"));if(void 0==w)return $.user_message("Die MailFrom-Adresse konnte nicht ermittelt werden!","warning"),-1;if(""==usrconfig)$.login("LOGINFIRST",function(){var n={},t=$("input[name=volume]").val();void 0!=t&&(n.volume=t);t=$("#msg").val();
void 0==t&&(t=$("input[name=msg]:checked").val());void 0!=t&&(n.msg=t);t=$("#msg2").val();void 0==t&&(t=$("input[name=msg2]:checked").val());void 0!=t&&(n.msg2=t);t=$("#msg3").val();void 0==t&&(t=$("input[name=msg3]:checked").val());void 0!=t&&(n.msg3=t);$("div[name=userinput]").find(":input").each(function(){n[$(this).attr("name")]=$(this).val()});t="<table>"+$.getfullbody()+"</table>";$.post("/ajax/mailorderto",{exemplar:JSON.stringify(d),fullbody:JSON.stringify(t),ppn:b,epn:c,mailto:r,mailtoname:q,
mailfrom:w,userinput:JSON.stringify(n),mailtyp:h,mailprint:l,mailsubject:f,userconfig:JSON.stringify(usrconfig)},function(m){if(0>m.status){if(-3==m.status)return $.user_message(m.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+m.error,"warning")}else answer.status="success",answer.message=configlang.ORDERSUCCESS,g.close()},"json")});else{var u={},p=$("input[name=volume]").val();void 0!=p&&(u.volume=p);p=$("#msg").val();void 0==p&&(p=$("input[name=msg]:checked").val());void 0!=
p&&(u.msg=p);p=$("#msg2").val();void 0==p&&(p=$("input[name=msg2]:checked").val());void 0!=p&&(u.msg2=p);p=$("#msg3").val();void 0==p&&(p=$("input[name=msg3]:checked").val());void 0!=p&&(u.msg3=p);$("div[name=userinput]").find(":input").each(function(){u[$(this).attr("name")]=$(this).val()});p="<table>"+$.getfullbody()+"</table>";$.post("/ajax/mailorderto",{exemplar:JSON.stringify(d),fullbody:JSON.stringify(p),ppn:b,epn:c,mailto:r,mailtoname:q,mailfrom:w,userinput:JSON.stringify(u),mailtyp:h,mailprint:l,
mailsubject:f,userconfig:JSON.stringify(usrconfig)},function(n){if(0>n.status){if(-3==n.status)return $.user_message(n.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+n.error,"warning")}else answer.status="success",answer.message=configlang.ORDERSUCCESS,g.close()},"json")}}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(g){dialogs.push(k)}});k.realize();k.getModalHeader().addClass("preview-header");k.setData("name","mailorder");k.open()};
$.checkmanfield=function(a){var b=$("#"+a).val();return void 0!=b&&b?b:($.user_message(configlang.REQUIREDFIELD+" : "+$("label[for="+a+"]").text(),"warning"),$("#"+a).focus(),"")};
$.ilorder=function(a,b,c,d){a=$("#"+b+" div.panel-heading").html();void 0==a&&(a=$("#related_"+b).html());var e=void 0!=d.method?d.method:"",h=void 0!=d.data1?d.data1:"",l="1",f={},k="",g="",r="",q="",w={};$.close_resolvers();var u=new BootstrapDialog({message:$('<div id="ilorderviewbody"></div>').load("/ajax/ilorderview/"+e+"/"+h,function(p,n,t){}),title:a,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(p){p.close();$.focus_first()}},{id:"ilbtn",
label:configlang.NEXTSTEP+" <span class='bootstrap-dialog-button-icon glyphicon glyphicon-chevron-right'></span>",cssClass:"fullview-button-color",action:function(p){$(this).blur();if(1==l){if(!(k=$.checkmanfield("iluser"))||!(g=$.checkmanfield("ilpassword"))||void 0!=$("#ilyear").val()&&!(r=$.checkmanfield("ilyear")))return;q=void 0!=$("#ilvolume").val()?$("#ilvolume").val():"";var n={};n.iluser=k;n.ilpassword=g;n.ilyear=r;n.ilvolume=q}if(2==l){var t=!1;n={};n.iluser=k;n.ilpassword=g;n.ilyear=r;
n.ilvolume=q;n.inputs=w;$.each(f,function(m,v){if("P3VLP"==m&&"copy"==e){if(v.man&&"P3VLPMIN"==$("input[name=P3VLPSEL]:checked").val()&&!$("#P3VLP").val())return $.user_message(configlang.REQUIREDFIELD+" : "+$("label[for=P3VLPMIN]").text(),"warning"),$("#P3VLP").focus(),t=!0,!1;var x=$("#ilmincosts").val();x=void 0==x?0:parseInt(x);var y=$("#P3VLP").val();y=void 0==y?0:parseInt(y);if("P3VLPMIN"==$("input[name=P3VLPSEL]:checked").val()&&y<x)return $.user_message(configlang.ENSUREMINCOSTS,"warning"),
$("#P3VLP").focus(),t=!0,!1;"P3VLPMIN"==$("input[name=P3VLPSEL]:checked").val()?v.man?n.inputs.mandatory[m]=$("#"+m).val():n.inputs.optional[m]=$("#"+m).val():v.man?n.inputs.mandatory[m]=$("#"+m+"0").val():n.inputs.optional[m]=$("#"+m+"0").val()}else if("P3VLZ"==m&&"copy"==e){if(v.man&&!$("#P3VLZ:checked").val())return $.user_message(configlang.REQUIREDFIELD+" : "+$("label[for=P3VLZ]").text(),"warning"),$("#P3VLZ").focus(),t=!0,!1}else{if(v.man&&!$("#"+m).val())return $.user_message(configlang.REQUIREDFIELD+
" : "+$("label[for="+m+"]").text(),"warning"),$("#"+m).focus(),t=!0,!1;v.man?n.inputs.mandatory[m]=$("#"+m).val():n.inputs.optional[m]=$("#"+m).val()}});if(t)return;if(g!=$("#P3VPW").val()){$.user_message(configlang.PASSWORDWRONG,"warning");$("#P3VPW").focus();return}}$.post("/ajax/ilorder",{exemplar:JSON.stringify(d),ppn:b,epn:c,userinput:JSON.stringify(n),iltyp:e,action:"runloan"+l},function(m){if(0>m.status)return 1==l?($("#ilorder_messagebar").append($("<div id='message_vfl' class='alert-message warning' data-alert></div>")),
$("#message_vfl").hide().html("<div class='panel panel-danger'><div class='panel-heading'>"+m.message+"</div></div>").fadeIn("fast")):$.user_message(m.message,"warning"),$("#ilstep1").hide(),u.getButton("ilbtn").hide(),!1;1==l?(void 0!=m.fields&&(f=m.fields,w=m.inputs,cands=m.cands,$.each(f,function(v,x){void 0!=$("#"+v)&&$("#"+v).val(x.value)}),$.each(cands,function(v,x){$("#ilsupps").append("<label>"+x+"</label><br />")}),$("#ilsupparea").show(),u.getButton("ilbtn").html(configlang.ILORDER)),$("#ilorderviewbody").slideUp("fast",
"swing",function(){$("#ilstep1").hide();r&&void 0!=$("#P3VLB")&&$("#P3VLB").val(r);q&&void 0!=$("#P3VLC")&&$("#P3VLC").val(q);$("#ilstep2").show();$("#ilorderviewbody").slideDown("fast")}),l=2):2==l&&(answer.status="success",answer.message=configlang.ILORDERSUCCESS+": "+m.ilorderno,m.more&&(answer.message+="<br />"+m.more),p.close())},"json")}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(p){dialogs.push(u);$("#iluser").focus()}});u.realize();u.getModalHeader().addClass("preview-header");
u.setData("name","ilorder");u.open()};
$.special=function(a,b,c,d){a=$("#"+b+" div.panel-heading").html();void 0==a&&(a=$("#related_"+b).html());var e="SPECIAL",h="SPECIALORDER",l="Magazinbestellung";$.close_resolvers();var f=new BootstrapDialog({message:$('<div id="specialviewbody"></div>').load("/ajax/specialview/"+b+"/"+c,function(k,g,r){e=$(k).wrapAll($("<div>")).parent().find("#specialbutton").attr("value");h=$(k).wrapAll($("<div>")).parent().find("#specialtyp").attr("value");l=$(k).wrapAll($("<div>")).parent().find("#specialsubject").attr("value");f.getButton("specialbtn").html("<span class='bootstrap-dialog-button-icon fa fa-map-marker'></span> "+
e)}),title:a,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(k){k.close();$.focus_first()}},{id:"specialbtn",icon:"fa fa-map-marker",label:" "+configlang.SPECIAL,cssClass:"fullview-button-color",action:function(k){$(this).blur();var g=$("input[name=special1]:checked").val();void 0==g&&(g=$("#special1").val());void 0==g&&(g=$("#special1").attr("value"));if(void 0==g)return $.user_message("Special1 konnte nicht ermittelt werden!","warning"),-1;var r=
$("input[name=special2]:checked").val();void 0==r&&(r=$("#special2").val());void 0==r&&(r=$("#special2").attr("value"));if(void 0==r)return $.user_message("Special2 konnte nicht ermittelt werden!","warning"),-1;var q=$("input[name=special3]:checked").val();void 0==q&&(q=$("#special3").val());void 0==q&&(q=$("#special3").attr("value"));if(void 0==q)return $.user_message("Special3 konnte nicht ermittelt werden!","warning"),-1;var w=$("input[name=special4]:checked").val();void 0==w&&(w=$("#special4").val());
void 0==w&&(w=$("#special4").attr("value"));if(void 0==w)return $.user_message("Special4 konnte nicht ermittelt werden!","warning"),-1;var u=$("input[name=special5]:checked").val();void 0==u&&(u=$("#special5").val());void 0==u&&(u=$("#special5").attr("value"));if(void 0==u)return $.user_message("Special5 konnte nicht ermittelt werden!","warning"),-1;var p=$("input[name=cld_min]").val();void 0==p&&(p=$("#cld_min").attr("value"));var n=$("input[name=cld_max]").val();void 0==n&&(n=$("#cld_max").attr("value"));
if(void 0!=p&&void 0!=n){if(""==p||0>=parseInt(p))return $.user_message(configlang.MANPAGEFROM,"warning"),-1;if(""==n||parseInt(n)<parseInt(p))return $.user_message(configlang.MANPAGETO,"warning"),-1}if(""==usrconfig)$.login("LOGINFIRST",function(){var m={exemplar:d,userinput:{special1:g,special2:r,special3:q,special4:w,special5:u,cld_min:p,cld_max:n},specialsubject:l,userconfig:usrconfig,epn:c};$.post("/ajax/libraryspecial",{action:h,ppnlist:JSON.stringify(b),fields:JSON.stringify(m)},function(v){if(0>
v.status){if(-3==v.status)return $.user_message(v.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+v.error,"warning")}else answer.status="success",answer.message=configlang.SPECIALSUCCESS,v.more&&(answer.message+="<br />"+v.more),k.close()},"json")});else{var t={exemplar:d,userinput:{special1:g,special2:r,special3:q,special4:w,special5:u,cld_min:p,cld_max:n},specialsubject:l,userconfig:usrconfig,epn:c};$.post("/ajax/libraryspecial",{action:h,ppnlist:JSON.stringify(b),fields:JSON.stringify(t)},
function(m){if(0>m.status){if(-3==m.status)return $.user_message(m.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+m.error,"warning")}else answer.status="success",answer.message=configlang.SPECIALSUCCESS,m.more&&(answer.message+="<br />"+m.more),k.close()},"json")}}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(k){dialogs.push(f)}});f.realize();f.getModalHeader().addClass("preview-header");f.setData("name","special");f.open()};
