...
class:"",publisher:"",toc:"",id:"",series:"",isn:""},settingsname,isMobile=window.matchMedia("only screen and (max-width: 760px)").matches,intervalID="",$JSONData,answer={status:"",message:""},resolver={fullview:null,google:null,link:null,simular:null,export:null,imgurl:null},SettingsTooltip=!1;$.is_Empty=function(a){if(null==a)return!0;if(0<a.length)return!1;if(0===a.length||"object"!==typeof a)return!0;for(var b in a)if(hasOwnProperty.call(a,b))return!1;return!0};$.focus_first=function(){};
...
$("#searchtext_md").val(b);facetvals.yearrange="[1000 TO "+((new Date).getFullYear()+1)+"]";facetvals.yearmin=1E3;facetvals.yearmax=(new Date).getFullYear()+1;facetvals.yearminstart=1E3;facetvals.yearmaxstart=(new Date).getFullYear()+1;facetvals.typ="total";facetvals.formats={};facetvals.sort="scoredesc"}Scroll=!1;searchvals.package=1;searchvals.search=searchvals.text;$.open_wait();$.search_results(a,function(){$.close_wait();0==results.hits&&$.spell_suggestion();$.push_state();$.focus_first()})};
...
g.thumbnail_url+"' title='"+configlang.GOOGLEBOOKS+"' data-tooltip='tooltip' data-placement='buttom'></a>"),$.sendimgurl(g.thumbnail_url,$("#ppn_"+a).data("ppn"),function(){}));g.hasOwnProperty("preview")&&"partial"==g.preview&&g.hasOwnProperty("preview_url")&&$("#isbn_"+a).append("<p></p><a target='_blank' href='"+g.preview_url+"'><img src='systemassets/lukida/img/google_preview_"+config.language+".gif'></a>")});$("#isbn_"+a).val(d);b()}}):b()}else b()};
$.sendimgurl=function(a,b,c){0==b.length||0==a.length?c():resolver.imgurl=$.ajax({url:"/ajax/imgurl/",method:"POST",data:{ppn:b,url:a},dataType:"JSON",success:function(d){c()}})};
...
buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(f){f.close();$.focus_first()}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(){dialogs.push(g)}});g.realize();g.getModalHeader().addClass("preview-header");g.setData("name","shelve");g.open()};
$.reservation=function(a,b,c,d){if(void 0!=d.id){var e=c="";for(f in d)if("action"!=f&&"typ"!=f&&"id"!=f&&"iln"!=f&&"class"!=f&&""!=d[f]&&"data"!=f.replace(/\d/g,""))if(""==e&&(e=f.replace(/\d/g,"")),""!=e&&e!=f.replace(/\d/g,"")&&("radio"!=e&&(c+="<br />"),e=f.replace(/\d/g,"")),"radio"==e)if("radio0"==f)c+=d[f]+"<br />";else{var g=d[f].split("|");c+="<div class='radio'><label><input type='radio' value='"+g[0]+"' name='counter'>"+g[1]+"</label></div>"}else"hidden"==e?c+="<input class='hidden' type='text' value='"+
...
action:function(l){$(this).blur();var h="";if($("input[name='counter']").length)if($("input[name='counter']").is(":hidden"))h=$("input[name='counter']:hidden").val();else{if(!$("input[name='counter']").is(":checked")){$.user_message(configlang.SELECTDESK,"warning");return}h=$("input[name='counter']:checked").val()}$.request(a,d.id,configlang.RESERVATIONSUCCESS,h,"reserve",function(q){"warning"==q.status?($.user_message(q.message,q.status),q.message="",q.status=""):l.close()})}}],onhidden:function(){dialogs.pop();
$.show_answer()},onshown:function(){dialogs.push(k)}});k.realize();k.getModalHeader().addClass("preview-header");k.setData("name","reservation");k.open()}};
...
var h="";if($("input[name='counter']").length)if($("input[name='counter']").is(":hidden"))h=$("input[name='counter']:hidden").val();else{if(!$("input[name='counter']").is(":checked")){$.user_message(configlang.SELECTDESK,"warning");return}h=$("input[name='counter']:checked").val()}$.request(a,d.id,configlang.ORDERSUCCESS,h,"order",function(q){"warning"==q.status?($.user_message(q.message,q.status),q.message="",q.status=""):l.close()})}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(){dialogs.push(k)}});
...
answer.message=c);g(answer)},"json")};
$.getfullbody=function(){var a="",b=dialogs.length;do $("#biblio_"+b).length&&($tmp=$("#biblio_"+b).clone(),$tmp.find("script").remove(),$tmp.find("button").removeAttr("onclick"),$tmp.find("button").removeAttr("class"),$tmp.find("button").each(function(){$(this).replaceWith("<p>"+$(this).html()+"</p>")}),$tmp.find("img").remove(),$tmp.find("span.fa").remove(),$tmp.find("span.bibsum_0down").remove(),$tmp.find("span.bibsum_1down").remove(),$tmp.find("span.bibsum_2down").remove(),$tmp.find("span.bibsum_3down").remove(),
$tmp.find("span.bibsum_0up").remove(),$tmp.find("span.bibsum_1up").remove(),$tmp.find("span.bibsum_2up").remove(),$tmp.find("span.bibsum_3up").remove(),$tmp.find("tr.discoverbibsum_0").remove(),$tmp.find("tr.discoverbibsum_1").remove(),$tmp.find("tr.discoverbibsum_2").remove(),$tmp.find("tr.discoverbibsum_3").remove(),$tmp.find("i.fa").remove(),$tmp.find("div.section_1").remove(),$tmp.find("div.section_2").remove(),$tmp.find("div.section_3").remove(),$tmp.find("div.section_4").remove(),$tmp.find("div").removeAttr("class"),
$tmp.find("div").removeAttr("id"),$tmp.find("div").removeAttr("data-ppn"),$tmp.find("div").removeAttr("data-isbn"),$tmp.find("table").removeAttr("class"),$tmp.find("a").removeAttr("class"),$tmp.find("a").removeAttr("href"),$tmp.find("a").removeAttr("target"),$tmp.find("a").removeAttr("tabindex"),$tmp.find("a").removeAttr("role"),$tmp.find("a").removeAttr("class"),$tmp.find("a").removeAttr("title"),$tmp.find("a").removeAttr("data-toggle"),$tmp.find("a").removeAttr("data-trigger"),$tmp.find("a").removeAttr("data-placement"),
$tmp.find("a").removeAttr("data-title"),$tmp.find("a").removeAttr("data-html"),$tmp.find("a").removeAttr("data-content"),$tmp.find("a").removeAttr("data-original-title"),a=$tmp.html()),b--;while(""==a&&0<=b);return a};
$.mailorder=function(a,b,c,d){a=$("#"+b+" div.panel-heading").html();void 0==a&&(a=$("#related_"+b).html());var e="ORDER",g="MAILORDER",f="Magazinbestellung";$.close_resolvers();var k=new BootstrapDialog({message:$('<div id="mailorderviewbody"></div>').load("/ajax/mailorderview/"+b+"/"+c,function(l,h,q){e=$(l).wrapAll($("<div>")).parent().find("#mailbutton").attr("value").toString();g=$(l).wrapAll($("<div>")).parent().find("#mailtyp").attr("value").toString();f=$(l).wrapAll($("<div>")).parent().find("#mailsubject").attr("value").toString();
...
void 0==q&&(q=$("#mailto").data("label"));if(void 0==h)return $.user_message("Die MailTo-Adresse konnte nicht ermittelt werden!","warning"),-1;var t=$("input[name=mailfrom]:checked").val();void 0==t&&(t=$("#mailfrom").attr("value"));if(void 0==t)return $.user_message("Die MailFrom-Adresse konnte nicht ermittelt werden!","warning"),-1;if(""==usrconfig)$.login("LOGINFIRST",function(){var r={},m=$("input[name=volume]").val();void 0!=m&&(r.volume=m);m=$("#msg").val();void 0==m&&(m=$("input[name=msg]:checked").val());
void 0!=m&&(r.msg=m);m=$("#msg2").val();void 0==m&&(m=$("input[name=msg2]:checked").val());void 0!=m&&(r.msg2=m);m=$("#msg3").val();void 0==m&&(m=$("input[name=msg3]:checked").val());void 0!=m&&(r.msg3=m);$("div[name=userinput]").find(":input").each(function(){r[$(this).attr("name")]=$(this).val()});m="<table>"+$.getfullbody()+"</table>";$.post("/ajax/mailorderto",{exemplar:JSON.stringify(d),fullbody:JSON.stringify(m),ppn:b,epn:c,mailto:h,mailtoname:q,mailfrom:t,userinput:JSON.stringify(r),mailtyp:g,
mailsubject:f,userconfig:JSON.stringify(usrconfig)},function(w){if(0>w.status){if(-3==w.status)return $.user_message(w.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+w.error,"warning")}else answer.status="success",answer.message=configlang.ORDERSUCCESS,l.close()},"json")});else{var u={},p=$("input[name=volume]").val();void 0!=p&&(u.volume=p);p=$("#msg").val();void 0==p&&(p=$("input[name=msg]:checked").val());void 0!=p&&(u.msg=p);p=$("#msg2").val();void 0==p&&(p=$("input[name=msg2]:checked").val());
void 0!=p&&(u.msg2=p);p=$("#msg3").val();void 0==p&&(p=$("input[name=msg3]:checked").val());void 0!=p&&(u.msg3=p);$("div[name=userinput]").find(":input").each(function(){u[$(this).attr("name")]=$(this).val()});p="<table>"+$.getfullbody()+"</table>";$.post("/ajax/mailorderto",{exemplar:JSON.stringify(d),fullbody:JSON.stringify(p),ppn:b,epn:c,mailto:h,mailtoname:q,mailfrom:t,userinput:JSON.stringify(u),mailtyp:g,mailsubject:f,userconfig:JSON.stringify(usrconfig)},function(r){if(0>r.status){if(-3==r.status)return $.user_message(r.error,
"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+r.error,"warning")}else answer.status="success",answer.message=configlang.ORDERSUCCESS,l.close()},"json")}}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(l){dialogs.push(k)}});k.realize();k.getModalHeader().addClass("preview-header");k.setData("name","mailorder");k.open()};
...
$.ilorder=function(a,b,c,d){a=$("#"+b+" div.panel-heading").html();void 0==a&&(a=$("#related_"+b).html());var e=void 0!=d.method?d.method:"",g=void 0!=d.data1?d.data1:"",f="1",k={},l="",h="",q="",t="",u={};$.close_resolvers();var p=new BootstrapDialog({message:$('<div id="ilorderviewbody"></div>').load("/ajax/ilorderview/"+e+"/"+g,function(r,m,w){}),title:a,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(r){r.close();$.focus_first()}},{id:"ilbtn",
label:configlang.NEXTSTEP+" <span class='bootstrap-dialog-button-icon glyphicon glyphicon-chevron-right'></span>",cssClass:"fullview-button-color",action:function(r){$(this).blur();if(1==f){if(!(l=$.checkmanfield("iluser"))||!(h=$.checkmanfield("ilpassword"))||void 0!=$("#ilyear").val()&&!(q=$.checkmanfield("ilyear")))return;t=void 0!=$("#ilvolume").val()?$("#ilvolume").val():"";var m={};m.iluser=l;m.ilpassword=h;m.ilyear=q;m.ilvolume=t}if(2==f){var w=!1;m={};m.iluser=l;m.ilpassword=h;m.ilyear=q;
m.ilvolume=t;m.inputs=u;$.each(k,function(n,v){if("P3VLP"==n&&"copy"==e){if(v.man&&"P3VLPMIN"==$("input[name=P3VLPSEL]:checked").val()&&!$("#P3VLP").val())return $.user_message(configlang.REQUIREDFIELD+" : "+$("label[for=P3VLPMIN]").text(),"warning"),$("#P3VLP").focus(),w=!0,!1;var x=$("#ilmincosts").val();x=void 0==x?0:parseInt(x);var y=$("#P3VLP").val();y=void 0==y?0:parseInt(y);if("P3VLPMIN"==$("input[name=P3VLPSEL]:checked").val()&&y<x)return $.user_message(configlang.ENSUREMINCOSTS,"warning"),
$("#P3VLP").focus(),w=!0,!1;"P3VLPMIN"==$("input[name=P3VLPSEL]:checked").val()?v.man?m.inputs.mandatory[n]=$("#"+n).val():m.inputs.optional[n]=$("#"+n).val():v.man?m.inputs.mandatory[n]=$("#"+n+"0").val():m.inputs.optional[n]=$("#"+n+"0").val()}else if("P3VLZ"==n&&"copy"==e){if(v.man&&!$("#P3VLZ:checked").val())return $.user_message(configlang.REQUIREDFIELD+" : "+$("label[for=P3VLZ]").text(),"warning"),$("#P3VLZ").focus(),w=!0,!1}else{if(v.man&&!$("#"+n).val())return $.user_message(configlang.REQUIREDFIELD+
" : "+$("label[for="+n+"]").text(),"warning"),$("#"+n).focus(),w=!0,!1;v.man?m.inputs.mandatory[n]=$("#"+n).val():m.inputs.optional[n]=$("#"+n).val()}});if(w)return;if(h!=$("#P3VPW").val()){$.user_message(configlang.PASSWORDWRONG,"warning");$("#P3VPW").focus();return}}$.post("/ajax/ilorder",{exemplar:JSON.stringify(d),ppn:b,epn:c,userinput:JSON.stringify(m),iltyp:e,action:"runloan"+f},function(n){if(0>n.status)return 1==f?($("#ilorder_messagebar").append($("<div id='message_vfl' class='alert-message warning' data-alert></div>")),
$("#message_vfl").hide().html("<div class='panel panel-danger'><div class='panel-heading'>"+n.message+"</div></div>").fadeIn("fast")):$.user_message(n.message,"warning"),$("#ilstep1").hide(),p.getButton("ilbtn").hide(),!1;1==f?(void 0!=n.fields&&(k=n.fields,u=n.inputs,cands=n.cands,$.each(k,function(v,x){void 0!=$("#"+v)&&$("#"+v).val(x.value)}),$.each(cands,function(v,x){$("#ilsupps").append("<label>"+x+"</label><br />")}),$("#ilsupparea").show(),p.getButton("ilbtn").html(configlang.ILORDER)),$("#ilorderviewbody").slideUp("fast",
"swing",function(){$("#ilstep1").hide();q&&void 0!=$("#P3VLB")&&$("#P3VLB").val(q);t&&void 0!=$("#P3VLC")&&$("#P3VLC").val(t);$("#ilstep2").show();$("#ilorderviewbody").slideDown("fast")}),f=2):2==f&&(answer.status="success",answer.message=configlang.ILORDERSUCCESS+": "+n.ilorderno,n.more&&(answer.message+="<br />"+n.more),r.close())},"json")}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(r){dialogs.push(p);$("#iluser").focus()}});p.realize();p.getModalHeader().addClass("preview-header");
p.setData("name","ilorder");p.open()};
$.special=function(a,b,c,d){a=$("#"+b+" div.panel-heading").html();void 0==a&&(a=$("#related_"+b).html());var e="SPECIAL",g="SPECIALORDER",f="Magazinbestellung";$.close_resolvers();var k=new BootstrapDialog({message:$('<div id="specialviewbody"></div>').load("/ajax/specialview/"+b+"/"+c,function(l,h,q){e=$(l).wrapAll($("<div>")).parent().find("#specialbutton").attr("value").toString();g=$(l).wrapAll($("<div>")).parent().find("#specialtyp").attr("value").toString();f=$(l).wrapAll($("<div>")).parent().find("#specialsubject").attr("value").toString();
k.getButton("specialbtn").html("<span class='bootstrap-dialog-button-icon fa fa-map-marker'></span> "+e)}),title:a,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(l){l.close();$.focus_first()}},{id:"specialbtn",icon:"fa fa-map-marker",label:" "+configlang.SPECIAL,cssClass:"fullview-button-color",action:function(l){$(this).blur();var h=$("input[name=special1]:checked").val();void 0==h&&(h=$("#special1").val());void 0==h&&(h=$("#special1").attr("value"));
if(void 0==h)return $.user_message("Special1 konnte nicht ermittelt werden!","warning"),-1;var q=$("input[name=special2]:checked").val();void 0==q&&(q=$("#special2").val());void 0==q&&(q=$("#special2").attr("value"));if(void 0==q)return $.user_message("Special2 konnte nicht ermittelt werden!","warning"),-1;var t=$("input[name=special3]:checked").val();void 0==t&&(t=$("#special3").val());void 0==t&&(t=$("#special3").attr("value"));if(void 0==t)return $.user_message("Special3 konnte nicht ermittelt werden!",
"warning"),-1;var u=$("input[name=special4]:checked").val();void 0==u&&(u=$("#special4").val());void 0==u&&(u=$("#special4").attr("value"));if(void 0==u)return $.user_message("Special4 konnte nicht ermittelt werden!","warning"),-1;var p=$("input[name=special5]:checked").val();void 0==p&&(p=$("#special5").val());void 0==p&&(p=$("#special5").attr("value"));if(void 0==p)return $.user_message("Special5 konnte nicht ermittelt werden!","warning"),-1;var r=$("input[name=cld_min]").val();void 0==r&&(r=$("#cld_min").attr("value"));
var m=$("input[name=cld_max]").val();void 0==m&&(m=$("#cld_max").attr("value"));if(void 0!=r&&void 0!=m){if(""==r||0>=parseInt(r))return $.user_message(configlang.MANPAGEFROM,"warning"),-1;if(""==m||parseInt(m)<parseInt(r))return $.user_message(configlang.MANPAGETO,"warning"),-1}if(""==usrconfig)$.login("LOGINFIRST",function(){var n={exemplar:d,userinput:{special1:h,special2:q,special3:t,special4:u,special5:p,cld_min:r,cld_max:m},specialsubject:f,userconfig:usrconfig,epn:c};$.post("/ajax/libraryspecial",
{action:g,ppnlist:JSON.stringify(b),fields:JSON.stringify(n)},function(v){if(0>v.status){if(-3==v.status)return $.user_message(v.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+v.error,"warning")}else answer.status="success",answer.message=configlang.SPECIALSUCCESS,v.more&&(answer.message+="<br />"+v.more),l.close()},"json")});else{var w={exemplar:d,userinput:{special1:h,special2:q,special3:t,special4:u,special5:p,cld_min:r,cld_max:m},specialsubject:f,userconfig:usrconfig,epn:c};
$.post("/ajax/libraryspecial",{action:g,ppnlist:JSON.stringify(b),fields:JSON.stringify(w)},function(n){if(0>n.status){if(-3==n.status)return $.user_message(n.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+n.error,"warning")}else answer.status="success",answer.message=configlang.SPECIALSUCCESS,n.more&&(answer.message+="<br />"+n.more),l.close()},"json")}}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(l){dialogs.push(k)}});k.realize();k.getModalHeader().addClass("preview-header");
k.setData("name","special");k.open()};$.cancel=function(a,b,c){$.close_resolvers();$.post("/ajax/cancel",{iln:a,uri:b},function(d){if(0>d.status){if(-3==d.status)return $.user_message(d.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+d.error,"warning")}else $("#reservation_"+c).hide(),$.user_message(configlang.CANCELSUCCESS,"success")},"json")};
