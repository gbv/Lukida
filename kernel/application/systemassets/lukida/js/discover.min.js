...
function(a,b){$(".Z3988").attr("title",b)})})};$.sortRelatedMedia=function(a,b,c){var d=$(a);a=$($(a).find(b),d);a.sort(function(a,b){var d=$(a).find(c).text().toLowerCase(),k=$(b).find(c).text().toLowerCase();return d>k?1:-1});$.each(a,function(a,b){d.append(b)})};
...
$.renew=function(){$(".status_renew").html("");$(".check_renew").each(function(a){if(1==$(this).prop("checked")){$(this).prop("checked",!1);a=$(this).data("item");var b=a.substring(a.lastIndexOf("$")+1);$.post("/ajax/renew",{uri:a},function(a){0>a.status?$("#status_renew_"+b).html('<font color="red"><a href="#" data-toggle="tooltip" title="'+a.error+'"><i class="fa fa-exclamation" aria-hidden="true"></i></font>'):($("#status_renew_"+b).html('<font color="green"><i class="fa fa-check" aria-hidden="true"></i></font>'),
$("#return_renew_"+b).html(a.endtime),$("#renewals_renew_"+b).html(a.renewals))},"json")}});$(".btn-renew").blur()};$.mark_check=function(a){$(".check_"+a).each(function(a){$(this).prop("checked",marcCheck)});marcCheck=!marcCheck;$(".btn-check-all").blur()};$.close_dialogs=function(){for(var a=dialogs.length-1;0<=a;a--)dialogs[a].close()};
...
0);$(".typeahead").typeahead({source:function(a,b){if(void 0==a)return b({});$.get("/ajax/getwords",{query:a},function(a){return b(a)},"json")},autoSelect:!1,delay:300,afterSelect:function(a){$.new_search("init",a)}});window.onpopstate=function(a){$.close_dialogs();$("#facets").fadeOut("fast").addClass("collapse");$("#results").addClass("collapse");$("#resultsonly").html("");$("#loading").fadeOut("fast").addClass("collapse");null!=a.state&&0<Object.keys(a.state).length?$.when($.pop_search(a.state.text,
a.state.facets)).done(function(){document.title=a.state.text+" | "+xtitle}):($("#start").fadeIn("fast").removeClass("collapse"),$("#searchtext_xs").val(""),$("#searchtext_md").val(""))};$(".yearstart.inlineedit").inlineEdit($('<input name="temp_start" type="number" min="1000" max="9999" />'),$("#hiddenyearstart"));$(".yearend.inlineedit").inlineEdit($('<input name="temp_end" type="number" min="1000" max="9999" />'),$("#hiddenyearend"));$("#hiddenyearstart").change(function(){facetvals.yearminstart=
