...
$.sortRelatedMedia=function(a,b,c){var d=$((a||window.event).currentTarget),e=$(d).text(),f=$(d).data("node"),g=$(d).data("sortdirection");a="desc"==g?"asc":"desc";"sButtonInv"!=d.attr("id")?(d=$(d).offsetParent().siblings("#sButtonDD"),$(d).find("span").eq(0).text(e+" "),e=$(d).siblings("#sButtonInv")):e=d;$(e).data("node",f);$(e).data("sortdirection",a);a=$(e).children("#iconInv");"#title"==f?"desc"==g?$(a).attr({"class":"glyphicon glyphicon-sort-by-alphabet-alt"}):$(a).attr({"class":"glyphicon glyphicon-sort-by-alphabet"}):
"desc"==g?$(a).attr({"class":"glyphicon glyphicon-sort-by-order-alt"}):$(a).attr({"class":"glyphicon glyphicon-sort-by-order"});var k=$(b);b=$($(b).find(c),k);b.sort(function(a,b){var c=$(a).find(f).text().toLowerCase(),d=$(b).find(f).text().toLowerCase();return"desc"==g?c<d?1:-1:c>d?1:-1});$.each(b,function(a,b){k.append(b)})};
$.searchrelatedmedia=function(a,b,c){$.when($.ajax({url:"/ajax/searchrelatedpubs/",method:"POST",data:{ppnlink:a,search:b},dataType:"JSON"})).then(function(a){0<a.hitspack?$(c).html(a.results):$(c).html("<div class='col-xs-12'><div class='well'>"+configlang.NOSIMULARHITS+"</div></div>")})};$.correctCover=function(a,b){a.onerror="";$(a).replaceWith("<span class='gbvicon'>"+b+"</span>")};
...
window.scrollTo(0,0);var a=$(".typeahead");a.typeahead({source:function(a,c){if(void 0==a||""==a.trim())return c({});$.get("/ajax/getwords",{query:a},function(a){return c(a)},"json")},autoSelect:!1,delay:300,afterSelect:function(a){$.new_search("init",a)}});a.keydown(function(a){console.log("Hir!");if(13==a.keyCode||9==a.keyCode){a=$(this).val().trim();var c=$(this).typeahead("getActive");c?(c=c.trim(),c==a?$.new_search("init",a):0<$(".navbar:visible ul.typeahead.dropdown-menu").find("li.active").length?
