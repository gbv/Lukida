...
var h="";if($("input[name='counter']").length)if($("input[name='counter']").is(":hidden"))h=$("input[name='counter']:hidden").val();else{if(!$("input[name='counter']").is(":checked")){$.user_message(configlang.SELECTDESK,"warning");return}h=$("input[name='counter']:checked").val()}$.request(a,d.id,configlang.RESERVATIONSUCCESS,h,"reserve",function(p){"warning"==p.status?($.user_message(p.message,p.status),p.message="",p.status=""):l.close()})}}],onhidden:function(){dialogs.pop();$.show_answer()},
...
"";if($("input[name='counter']").length)if($("input[name='counter']").is(":hidden"))h=$("input[name='counter']:hidden").val();else{if(!$("input[name='counter']").is(":checked")){$.user_message(configlang.SELECTDESK,"warning");return}h=$("input[name='counter']:checked").val()}$.request(a,d.id,configlang.ORDERSUCCESS,h,"order",function(p){"warning"==p.status?($.user_message(p.message,p.status),p.message="",p.status=""):l.close()})}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(){dialogs.push(k)}});
...
$.mailorder=function(a,b,c,d){a=$("#"+b+" div.panel-heading").html();void 0==a&&(a=$("#related_"+b).html());var e="ORDER",g="MAILORDER",f="Magazinbestellung";$.close_resolvers();var k=new BootstrapDialog({message:$('<div id="mailorderviewbody"></div>').load("/ajax/mailorderview/"+b+"/"+c,function(l,h,p){e=$(l).wrapAll($("<div>")).parent().find("#mailbutton").attr("value").toString();g=$(l).wrapAll($("<div>")).parent().find("#mailtyp").attr("value").toString();f=$(l).wrapAll($("<div>")).parent().find("#mailsubject").attr("value").toString();
k.getButton("mailbtn").html("<span class='bootstrap-dialog-button-icon fa fa-map-marker'></span> "+e)}),title:a,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(l){l.close();$.focus_first()}},{id:"mailbtn",icon:"fa fa-map-marker",label:" "+configlang.ORDER,cssClass:"fullview-button-color",action:function(l){$(this).blur();var h=$("input[name=mailto]:checked").val(),p=$("input[name=mailto]:checked").parent().text();void 0==h&&(h=$("#mailto").attr("value"));
void 0==p&&(p=$("#mailto").data("label"));if(void 0==h)return $.user_message("Die MailTo-Adresse konnte nicht ermittelt werden!","warning"),-1;var r=$("input[name=mailfrom]:checked").val();void 0==r&&(r=$("#mailfrom").attr("value"));if(void 0==r)return $.user_message("Die MailFrom-Adresse konnte nicht ermittelt werden!","warning"),-1;if(""==usrconfig)$.login("LOGINFIRST",function(){var q={},m=$("input[name=volume]").val();void 0!=m&&(q.volume=m);m=$("#msg").val();void 0==m&&(m=$("input[name=msg]:checked").val());
void 0!=m&&(q.msg=m);m=$("#msg2").val();void 0==m&&(m=$("input[name=msg2]:checked").val());void 0!=m&&(q.msg2=m);m=$("#msg3").val();void 0==m&&(m=$("input[name=msg3]:checked").val());void 0!=m&&(q.msg3=m);m="<table>"+$.getfullbody()+"</table>";$.post("/ajax/mailorderto",{exemplar:JSON.stringify(d),fullbody:JSON.stringify(m),ppn:b,epn:c,mailto:h,mailtoname:p,mailfrom:r,userinput:JSON.stringify(q),mailtyp:g,mailsubject:f,userconfig:JSON.stringify(usrconfig)},function(u){if(0>u.status){if(-3==u.status)return $.user_message(u.error,
"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+u.error,"warning")}else answer.status="success",answer.message=configlang.ORDERSUCCESS,l.close()},"json")});else{var t={},n=$("input[name=volume]").val();void 0!=n&&(t.volume=n);n=$("#msg").val();void 0==n&&(n=$("input[name=msg]:checked").val());void 0!=n&&(t.msg=n);n=$("#msg2").val();void 0==n&&(n=$("input[name=msg2]:checked").val());void 0!=n&&(t.msg2=n);n=$("#msg3").val();void 0==n&&(n=$("input[name=msg3]:checked").val());void 0!=
n&&(t.msg3=n);n="<table>"+$.getfullbody()+"</table>";$.post("/ajax/mailorderto",{exemplar:JSON.stringify(d),fullbody:JSON.stringify(n),ppn:b,epn:c,mailto:h,mailtoname:p,mailfrom:r,userinput:JSON.stringify(t),mailtyp:g,mailsubject:f,userconfig:JSON.stringify(usrconfig)},function(q){if(0>q.status){if(-3==q.status)return $.user_message(q.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+q.error,"warning")}else answer.status="success",answer.message=configlang.ORDERSUCCESS,l.close()},
"json")}}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(l){dialogs.push(k)}});k.realize();k.getModalHeader().addClass("preview-header");k.setData("name","mailorder");k.open()};
