...
window.scrollTo(0,0);var a=$(".typeahead");a.typeahead({source:function(a,c){if(void 0==a||""==a.trim())return c({});$.get("/ajax/getwords",{query:a},function(a){return c(a)},"json")},autoSelect:!1,delay:300});a.keydown(function(a){if(13==a.keyCode||9==a.keyCode){a=$(this).val().trim();var c=$(this).typeahead("getActive").trim();c?c==a?$.new_search("init",a):0<$(".navbar:visible ul.typeahead.dropdown-menu").find("li.active").length?$.new_search("init",$("ul.typeahead.dropdown-menu:visible").find("li.active a").html().replace(/(<([^>]+)>)/ig,
"").trim()):$.new_search("init",a):$.new_search("init",a);event.preventDefault()}});window.onpopstate=function(a){$.close_dialogs();$("#facets").fadeOut("fast").addClass("collapse");$("#results").addClass("collapse");$("#resultsonly").html("");$("#loading").fadeOut("fast").addClass("collapse");null!=a.state&&0<Object.keys(a.state).length?($.init_client(a.state.sets),appstate.first=!0,$.new_search("initfacets",a.state.text),document.title=a.state.text+" | "+xtitle):($("#start").fadeIn("fast").removeClass("collapse"),
