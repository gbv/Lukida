...
$jscomp.polyfill("Array.prototype.find",function(a){return a?a:function(b,a){return $jscomp.findInternal(this,b,a).v}},"es6","es3");$jscomp.SYMBOL_PREFIX="jscomp_symbol_";$jscomp.initSymbol=function(){$jscomp.initSymbol=function(){};$jscomp.global.Symbol||($jscomp.global.Symbol=$jscomp.Symbol)};$jscomp.Symbol=function(){var a=0;return function(b){return $jscomp.SYMBOL_PREFIX+(b||"")+a++}}();
...
$.opened_fullview=function(a,b){$.replace_state(a);$.close_resolvers();$.googleresolver(b,function(){$.linkresolver(a,b,function(){$.simularpublications(a,b,function(){$.exportlink(a,function(){})})})})};
...
$.open_fullview=function(a,b,c,e){void 0===b&&(b=results.ppnlist);void 0===e&&(e=a);void 0===c&&(c="main");"main"!=c&&(b=jQuery.map(b,function(a,b){return a.toString()}));var d=dialogs.length,f=a in favors?configlang.CHECKLISTREMOVE:configlang.CHECKLIST,h=a in favors?"fa fa-star-o":"fa fa-star",g="publications"==c?$("#related_"+e).html():$("#"+e+" div.panel-heading").html(),k=new BootstrapDialog({message:'<div id="fullbody_'+d+'"><div class="outercircle"></div><div class="innercircle"></div></div>',
...
1?$.search_next_pack(function(){b=results.ppnlist;a=b[b.indexOf(a)+1];$("#fullbody_"+d).slideUp("fast","swing",function(){$.set_fullviewppn(a,d,c,function(){$("#fullbody_"+d).slideDown("fast")})})}):b.indexOf(a)<b.length-1?(a=b[b.indexOf(a)+1],$("#fullbody_"+d).slideUp("fast","swing",function(){$.set_fullviewppn(a,d,c,function(){$("#fullbody_"+d).slideDown("fast")})})):$.user_message(configlang.LASTOBJECT,"warning")}},{id:"checklistbtn_xs_"+d,label:' <span class="text">'+f+"</span>",icon:h,cssClass:"btn-default checklist hidden-md hidden-lg col-xs-4 col-sm-4 LeftOffset2",
action:function(b){$(this).blur();a in favors?(delete favors[a],delete favormails[a],delete favorprint[a],$.user_message(configlang.CHECKLISTREMOVESUCCESS,"success"),b='<span class="bootstrap-dialog-button-icon fa fa-star"></span> <span class="text">'+configlang.CHECKLIST+"</span>"):(favors[a]=$("#"+a).clone().removeClass("medium col-xs-12 col-sm-6 col-md-4 col-lg-3","col-md-6").addClass("merk col-md-6"),$(favors[a]).find("a").attr("href","javascript:$.open_fullview('"+a+"',Object.keys(favors));"),
...
"total";facetvals.formats={};facetvals.yearmin=facetvals.yearminbase;facetvals.yearmax=facetvals.yearmaxbase;facetvals.yearminstart=facetvals.yearminbase;facetvals.yearmaxstart=facetvals.yearmaxbase;facetvals.yearrange="["+facetvals.yearminbase+" TO "+facetvals.yearmaxbase+"]";c==JSON.stringify(settings)&&f==facetvals.yearrange&&e==facetvals.datapool&&d==facetvals.phonetic||$.new_search("init",$("#searchtext_md").val());a.close()})}}],onhidden:function(){dialogs.pop();$.show_answer();"undefined"!=
typeof slider3&&slider3.noUiSlider.destroy()},onshow:function(){config.lbs||h.getButton("loadsettings").hide();config.lbs||h.getButton("savesettings").hide()},onshown:function(){slider3=document.getElementById("pubyear3");noUiSlider.create(slider3,b);slider3.style.margin="10px 20px 10px 20px";slider3.noUiSlider.on("update",function(a,b){$(".yearstart2").html(Math.floor(a[0]));$(".yearend2").html(Math.floor(a[1]));facetvals.yearminbase=Math.floor(a[0]);facetvals.yearmaxbase=Math.floor(a[1])});"global"==
...
dialogs.push(h)}});h.realize();h.getModalHeader().addClass("preview-header");h.setData("name","settings");h.open()};
...
"warning"),""==user?$("#kennung").focus():$("#passwort").focus(),-1;$.post("/ajax/login",{user:user,pw:pw},function(b){"-1"!=b?(a.close(),$(".nologinarea").addClass("hide"),$(".loginarea").removeClass("hide"),usrconfig=b,timer=!0,$.startTimer(),answer.status="success",answer.message=configlang.LOGINSUCCESS.replace("{firstname}",b.firstname).replace("{lastname}",b.lastname),b.message&&(c="warning",e=b.messagetext)):($.user_message(configlang.LOGINERROR,"warning"),$("#kennung").focus(),usrconfig="");
$.focus_first()},"json")}}],onhidden:function(){dialogs.pop();$.show_answer();""!=c&&$.user_message(e,c,1E4);b()},onshown:function(){dialogs.push(d);"1"==config.devmode&&($("#kennung").val(config.devuser),$("#passwort").val(config.devpassword));$("#kennung").focus();$(".keyenter").keypress(function(a){13==a.which&&($(this).blur(),$("#loginbutton").focus().click())});void 0!=a&&""!=a&&$.user_message(configlang[a],"warning")}});d.realize();d.getModalHeader().addClass("preview-header");d.setData("name",
...
$.reservation=function(a,b,c){if(void 0!=c.id){var e=b="";for(f in c)if("action"!=f&&"typ"!=f&&"id"!=f&&""!=c[f])if(""==e&&(e=f.replace(/\d/g,"")),""!=e&&e!=f.replace(/\d/g,"")&&("radio"!=e&&(b+="<br />"),e=f.replace(/\d/g,"")),"radio"==e)if("radio0"==f)b+=c[f]+"<br />";else{var d=c[f].split("|");b+="<div class='radio'><label><input type='radio' value='"+d[0]+"' name='counter'>"+d[1]+"</label></div>"}else b="hidden"==e?b+("<input class='hidden' type='text' value='"+c[f]+"' name='counter'>"):b+(c[f].replace(/%lt%/g,
"&lt;").replace(/%gt%/g,"&gt;")+"<br />");b+='<div id="reservation_messagebar"></div>';var f=$("#"+a+" div.panel-heading").html();void 0==f&&(f=$("#related_"+a).html());var h=new BootstrapDialog({message:b,title:f,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(a){a.close();$.focus_first()}},{icon:"fa fa-map-marker",label:" "+configlang.RESERVATION,cssClass:"fullview-button-color",action:function(a){$(this).blur();var b="";if($("input[name='counter']").length)if($("input[name='counter']").is(":hidden"))b=
$("input[name='counter']:hidden").val();else{if(!$("input[name='counter']").is(":checked")){$.user_message(configlang.SELECTDESK,"warning");return}b=$("input[name='counter']:checked").val()}$.request(c.id,configlang.RESERVATIONSUCCESS,b,"reserve",function(b){"warning"==b.status?($.user_message(b.message,b.status),b.message="",b.status=""):a.close()})}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(){dialogs.push(h)}});h.realize();h.getModalHeader().addClass("preview-header");
h.setData("name","reservation");h.open()}};
$.order=function(a,b,c){if(void 0!=c.id){var e=b="";for(f in c)if("action"!=f&&"typ"!=f&&"id"!=f&&""!=c[f])if(""==e&&(e=f.replace(/\d/g,"")),""!=e&&e!=f.replace(/\d/g,"")&&("radio"!=e&&(b+="<br />"),e=f.replace(/\d/g,"")),"radio"==e)if("radio0"==f)b+=c[f]+"<br />";else{var d=c[f].split("|");b+="<div class='radio'><label><input type='radio' value='"+d[0]+"' name='counter'>"+d[1]+"</label></div>"}else b="hidden"==e?b+("<input class='hidden' type='text' value='"+c[f]+"' name='counter'>"):b+(c[f].replace(/%lt%/g,
...
$("input[name='counter']:hidden").val();else{if(!$("input[name='counter']").is(":checked")){$.user_message(configlang.SELECTDESK,"warning");return}b=$("input[name='counter']:checked").val()}$.request(c.id,configlang.ORDERSUCCESS,b,"order",function(b){"warning"==b.status?($.user_message(b.message,b.status),b.message="",b.status=""):a.close()})}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(){dialogs.push(h)}});h.realize();h.getModalHeader().addClass("preview-header");h.setData("name",
"order");h.open()}};
$.request=function(a,b,c,e,d){""==usrconfig?$.login("LOGINFIRST",function(){$.post("/ajax/request",{uri:a,desk:c,action:e},function(a){0>a.status?(answer.status="warning",answer.message=configlang.PROCESSSTOPPED+"<br />"+a.error):(answer.status="success",answer.message=b);d(answer)},"json")}):$.post("/ajax/request",{uri:a,desk:c,action:e},function(a){0>a.status?(answer.status="warning",answer.message=configlang.PROCESSSTOPPED+"<br />"+a.error):(answer.status="success",answer.message=b);d(answer)},
"json")};
$.mailorder=function(a,b,c){var e=$("#"+a+" div.panel-heading").html();void 0==e&&(e=$("#related_"+a).html());var d="ORDER",f="MAILORDER",h="Magazinbestellung",g=new BootstrapDialog({message:$('<div id="mailorderviewbody"></div>').load("/ajax/mailorderview/"+a+"/"+b,function(a,b,c){d=$(a).wrapAll($("<div>")).parent().find("#mailbutton").attr("value").toString();f=$(a).wrapAll($("<div>")).parent().find("#mailtyp").attr("value").toString();h=$(a).wrapAll($("<div>")).parent().find("#mailsubject").attr("value").toString();g.getButton("mailbtn").html("<span class='bootstrap-dialog-button-icon fa fa-map-marker'></span> "+
d)}),title:e,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(a){a.close();$.focus_first()}},{id:"mailbtn",icon:"fa fa-map-marker",label:" "+configlang.ORDER,cssClass:"fullview-button-color",action:function(b){$(this).blur();var d=$("input[name=mailto]:checked").val(),e=$("input[name=mailto]:checked").data("label");void 0==d&&(d=$("#mailto").attr("value"));void 0==e&&(e=$("#mailto").data("label"));if(void 0==d)return $.user_message("Die MailTo-Adresse konnte nicht ermittelt werden!",
"warning"),-1;var g=$("input[name=mailfrom]:checked").val();void 0==g&&(g=$("#mailfrom").attr("value"));if(void 0==g)return $.user_message("Die MailFrom-Adresse konnte nicht ermittelt werden!","warning"),-1;if(""==usrconfig)$.login("LOGINFIRST",function(){var k={},l=$("input[name=volume]").val();void 0!=l&&(k.volume=l);l="<table>"+$("#biblio_"+(dialogs.length-2)).html()+"</table>";$.post("/ajax/mailorderto",{exemplar:JSON.stringify(c),fullbody:JSON.stringify(l),ppn:a,mailto:d,mailtoname:e,mailfrom:g,
userinput:JSON.stringify(k),mailtyp:f,mailsubject:h},function(a){if(0>a.status){if(-3==a.status)return $.user_message(a.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+a.error,"warning")}else answer.status="success",answer.message=configlang.ORDERSUCCESS,b.close()},"json")});else{var k={},l=$("input[name=volume]").val();void 0!=l&&(k.volume=l);l="<table>"+$("#biblio_"+(dialogs.length-2)).html()+"</table>";$.post("/ajax/mailorderto",{exemplar:JSON.stringify(c),fullbody:JSON.stringify(l),
ppn:a,mailto:d,mailtoname:e,mailfrom:g,userinput:JSON.stringify(k),mailtyp:f,mailsubject:h},function(a){if(0>a.status){if(-3==a.status)return $.user_message(a.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+a.error,"warning")}else answer.status="success",answer.message=configlang.ORDERSUCCESS,b.close()},"json")}}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(a){dialogs.push(g)}});g.realize();g.getModalHeader().addClass("preview-header");g.setData("name",
"mailorder");g.open()};$.cancel=function(a,b){$.post("/ajax/cancel",{uri:a},function(a){if(0>a.status){if(-3==a.status)return $.user_message(a.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+a.error,"warning")}else $("#reservation_"+b).hide(),$.user_message(configlang.CANCELSUCCESS,"success")},"json")};
