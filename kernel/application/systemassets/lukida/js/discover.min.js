...
$.format_hits=function(a){return 1E6<a?String(Math.round(a/1E4)/100).replace(".",",")+" Mio. ":1E3<a?String(Math.round(a/10)/100).replace(".",",")+" Tsd. ":a};
$.open_password=function(a){var b=new BootstrapDialog({message:'<div class="form-horizontal"><div class="form-group"><label for="pwpasswort" class="col-sm-3 control-label">'+configlang.PASSWORD+'</label><div class="col-sm-9"><input type="password" class="form-control keyenter" id="pwpasswort" placeholder="'+configlang.PASSWORD+'"></div></div><div id="pw_messagebar"></div></div>',title:configlang.MODULEACCESS,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(a){a.close();
$.focus_first()}},{icon:"glyphicon glyphicon-log-in",label:" "+configlang.LOGIN,cssClass:"fullview-button-color",id:"pwbutton",action:function(b){$(this).blur();pw=$("#pwpasswort").val().trim();if(""==pw)return $.user_message(configlang.PASSWORDWRONG,"warning"),$("#pwpasswort").focus(),-1;$.post("/ajax/checkpw",{module:a,pw:pw},function(a){"1"==a.status?window.location.href=urlport+window.location.host+"/librarymodule":($.user_message(configlang.LOGINERROR,"warning"),$("#kennung").focus());$.focus_first()},
"json")}}],onhidden:function(){dialogs.pop();callback()},onshown:function(){dialogs.push(b);$(".keyenter").keypress(function(a){13==a.which&&($(this).blur(),$("#pwbutton").focus().click())});$("#pwpasswort").focus()}});b.realize();b.getModalHeader().addClass("preview-header");b.setData("name","pw");b.open()};
$.command=function(a){"library"==a.toLowerCase()?$.open_password("library"):$.post("/ajax/command",{cmd:a},function(b){$.user_message("Befehl "+a+" erfolgreich ausgef&uuml;hrt","success");$.focus_first()},"json")};
