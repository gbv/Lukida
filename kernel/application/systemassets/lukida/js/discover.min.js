...
$.close_wait=function(){waitdialog.close()};$.openInNewTab=function(a){window.open(a,"_blank").focus()};$.validateEmail=function(a){return/^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/.test(a)};$.cleanArray=function(a){for(var b=0;b<a.length;b++)void 0==a[b].trim()||""==a[b].trim()?(a.splice(b,1),b--):a[b]=a[b].trim();return a};
$.SetQuery=function(){querystate?0<results.hits?($("#querybar_querybar").hide(),$("#results_querybar").show()):($("#querybar_querybar").show(),$("#results_querybar").hide()):($("#querybar_querybar").hide(),$("#results_querybar").hide())};
...
d.close())})}}],onhidden:function(){dialogs.pop();$.show_answer()},onshown:function(){dialogs.push(e);$("#username").focus()}});e.realize();e.getModalHeader().addClass("preview-header");e.setData("name","mail");e.open()}};
...
c);h(answer)},"json")};$.getfullbody=function(){var a="",b=dialogs.length;do $("#biblio_"+b).length&&(a=$("#biblio_"+b).html()),b--;while(""==a&&0<=b);return a};
...
void 0==d&&(d=$("#mailto").data("label"));if(void 0==c)return $.user_message("Die MailTo-Adresse konnte nicht ermittelt werden!","warning"),-1;var g=$("input[name=mailfrom]:checked").val();void 0==g&&(g=$("#mailfrom").attr("value"));if(void 0==g)return $.user_message("Die MailFrom-Adresse konnte nicht ermittelt werden!","warning"),-1;if(""==usrconfig)$.login("LOGINFIRST",function(){var k={},l=$("input[name=volume]").val();void 0!=l&&(k.volume=l);l="<table>"+$.getfullbody()+"</table>";$.post("/ajax/mailorderto",
{exemplar:JSON.stringify(e),fullbody:JSON.stringify(l),ppn:b,mailto:c,mailtoname:d,mailfrom:g,userinput:JSON.stringify(k),mailtyp:h,mailsubject:f,userconfig:JSON.stringify(usrconfig)},function(b){if(0>b.status){if(-3==b.status)return $.user_message(b.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+b.error,"warning")}else answer.status="success",answer.message=configlang.ORDERSUCCESS,a.close()},"json")});else{var k={},l=$("input[name=volume]").val();void 0!=l&&(k.volume=l);l=
"<table>"+$.getfullbody()+"</table>";$.post("/ajax/mailorderto",{exemplar:JSON.stringify(e),fullbody:JSON.stringify(l),ppn:b,mailto:c,mailtoname:d,mailfrom:g,userinput:JSON.stringify(k),mailtyp:h,mailsubject:f,userconfig:JSON.stringify(usrconfig)},function(b){if(0>b.status){if(-3==b.status)return $.user_message(b.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+b.error,"warning")}else answer.status="success",answer.message=configlang.ORDERSUCCESS,a.close()},"json")}}}],onhidden:function(){dialogs.pop();
$.show_answer()},onshown:function(a){dialogs.push(g)}});g.realize();g.getModalHeader().addClass("preview-header");g.setData("name","mailorder");g.open()};$.cancel=function(a,b,c){$.post("/ajax/cancel",{iln:a,uri:b},function(a){if(0>a.status){if(-3==a.status)return $.user_message(a.error,"warning"),!1;$.user_message(configlang.PROCESSSTOPPED+"<br />"+a.error,"warning")}else $("#reservation_"+c).hide(),$.user_message(configlang.CANCELSUCCESS,"success")},"json")};
