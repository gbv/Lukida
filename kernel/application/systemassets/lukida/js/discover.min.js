...
label:" "+configlang.CANCEL,action:function(a){a.close();$.focus_first()}},{icon:"fa fa-map-marker",label:" "+configlang.RESERVATION,cssClass:"fullview-button-color",action:function(a){$(this).blur();""==usrconfig?$.login("LOGINFIRST",function(){$.post("/ajax/request",{uri:c.id},function(b){0>b.status?(f="warning",g=configlang.PROCESSSTOPPED+"<br />"+b.error):(f="success",g=configlang.RESERVATIONSUCCESS);a.close()},"json")}):$.post("/ajax/request",{uri:c.id},function(b){if(0>b.status){if(-3==b.status)return $.user_message(b.error,
"warning"),!1;f="warning";g=configlang.PROCESSSTOPPED+"<br />"+b.error}else f="success",g=configlang.RESERVATIONSUCCESS;a.close()},"json")}}],onhidden:function(){dialogs.pop();""!=f&&$.user_message(g,f)},onshown:function(){dialogs.push(k)}});k.realize();k.getModalHeader().addClass("preview-header");k.setData("name","reservation");k.open()};
...
{uri:c.id},function(b){if(-3==b.status)return $.user_message(b.error,"warning"),!1;f="warning";g=configlang.PROCESSSTOPPED+"<br />"+b.error;a.close()},"json")}}],onhidden:function(){dialogs.pop();""!=f&&$.user_message(g,f)},onshown:function(){dialogs.push(k)}});k.realize();k.getModalHeader().addClass("preview-header");k.setData("name","order");k.open()}};
