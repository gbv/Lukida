var urlport=location.protocol+"//",configlang={},config={},messid=0,params={areas:[],start:moment().subtract(8,"days").format("YYYY-MM-DD"),end:moment().subtract(1,"days").format("YYYY-MM-DD")},chart="searches",bstacked=!1,access=!1;$.go=function(a){"discover"==a&&(window.location.href=urlport+window.location.host)};$.refresh=function(){$.set_chart(chart)};
...
$.set_chart=function(a){access?(chart!=a&&($("#libraryoption").is(":visible")&&$("#libraryoption").slideUp("slow",function(){$("#optionup").toggleClass("collapse");$("#optiondown").toggleClass("collapse")}),$("#libraryoption").find(".well").hide(),$("#libraryoption").find("."+a).show(),$("#libraryhelp").is(":visible")&&$("#libraryhelp").slideUp("slow",function(){$("#helpup").toggleClass("collapse");$("#helpdown").toggleClass("collapse")}),$("#libraryhelp").find(".well").hide(),$("#libraryhelp").find("."+
...
window.myChart.destroy();var b=$("#chart");window.myChart=new Chart(b,{type:"line",data:a.data,options:{title:a.title,scales:{yAxes:[{ticks:{beginAtZero:!0},stacked:bstacked}]}}})}})):$.open_password()};
