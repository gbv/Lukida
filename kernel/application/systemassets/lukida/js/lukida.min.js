...
{ppnlist:[],language:"",layout:""},favors={},favormails={},favorprint={},idleTimer,time2warn=1080,time2kill=120,timer=!1,urlport=location.protocol+"//";$.is_Empty=function(a){if(null==a)return!0;if(0<a.length)return!1;if(0===a.length||"object"!==typeof a)return!0;for(var b in a)if(hasOwnProperty.call(a,b))return!1;return!0};
...
"typ"==a&&(facetvals[a]=b,facetvals.formats={},$(".numformat").html(""),$("input.facetformat:checkbox").attr("checked",!1),$.new_search("facet_typ"));"format"==a&&(d?(facetvals.formats[b]=d,$.new_search("facet_format_checked")):(delete facetvals.formats[b],$.new_search("facet_format_unchecked")));$.post("/ajax/statsclient",{typ:"Facet",name:a,range:"day"})};
$.link_search=function(a,b){""!=b&&(""!=a&&(b="id"!=a&&"year"!=a?a+"('"+b+"')":a+"("+b.replace(/^\D+/g,"")+")"),$.close_dialogs(),$.new_search("link",b))};
$.push_state=function(){if(appstate.pushurl||appstate.first){var a=$.is_Empty(statevals)?{facetvals:facetvals,statevals:{language:config.language,layout:config.layout,ppnlist:{}}}:{facetvals:facetvals,statevals:{language:config.language,layout:config.layout,ppnlist:statevals.ppnlist}},b=searchvals.text.replace(/"/g,"'").replace(/\*/g,"{star}").replace(/\//g,"{slash}").replace(/</g,"{st}").replace(/>/g,"{gt}").replace(/:/g,"{colon}"),b=urlport+window.location.host+"/"+encodeURIComponent(b),b=b+("/"+
window.btoa(unescape(encodeURIComponent(JSON.stringify(a)))));appstate.first?(window.history.replaceState({text:searchvals.text,facets:facetvals,states:statevals},searchvals.text.substring(0,20)+" | "+xtitle,b),appstate.first=!1):window.history.pushState({text:searchvals.text,facets:facetvals,states:statevals},searchvals.text.substring(0,20)+" | "+xtitle,b)}};$.navbar_search=function(a){a=$(a).val();$.new_search("init",a)};
