var Scroll=!1,Height=$(document).height(),title="",user,pw,status,username,mailfrom,mailto,msg,config,usrconfig="",configlang,xtitle=document.title,slider,slider2,dialogs=[],waitdialog,messid=0,results={hits:0,hitspack:0,hitscurr:0,qtime:"",ppnlist:{}},searchvals={text:"",last:"","package":1,inittext:"",initfacets:{}},facetvals={datapool:"",yearrange:"",yearmin:"",yearmax:"",yearminstart:"",yearmaxstart:"",typ:"total",formats:{}},favors={},favormails={},idleTimer,time2warn=1080,time2kill=120,timer=
!1,urlport=location.protocol+"//";$.cookie("theme")&&$("[rel='"+$.cookie("theme")+"']").parent().addClass("active");
...
"",facetvals.yearmax="",facetvals.yearminstart="",facetvals.yearmaxstart="",facetvals.typ="total",facetvals.formats={},$("#hiddenyearstart").val(""),$("#hiddenyearend").val(""));document.title=xtitle+" "+searchvals.text}if("initfacets"!=a){if(0<=searchvals.text.search(":"))var c=searchvals.text.split(":"),e=c[0],c=c[1];else e="",c=searchvals.text;"*"==c&&(c="{star}");c=urlport+window.location.host+"/"+encodeURIComponent(c);c=c+(""!=e?"/"+e:"/{}")+("/"+window.btoa(unescape(encodeURIComponent(JSON.stringify(facetvals)))));
window.history.pushState({text:searchvals.text,facets:facetvals},"",c)}Scroll=!1;searchvals["package"]=1;$.open_wait();$.search_results(a,function(){0==results.hits&&$.user_message(configlang.NOHITS,"warning");$.close_wait();$.focus_first()})};$.search_next_pack=function(a){Scroll=!1;$("#loading").removeClass("hideLoading").addClass("displayLoading");searchvals["package"]++;$.search_results("adddata",function(){$("#loading").removeClass("displayLoading").addClass("hideLoading");a()})};
...
$.opened_fullview=function(a,b){(0<$("#linkresolver_"+b).parent().length||0<$("#linkresolvercontainer_"+b).parent().length)&&$.when($.ajax({url:"/ajax/linkresolver/",method:"POST",data:{ppn:a},dataType:"JSON"})).then(function(a){var e="";$.each(a,function(a,b){e+="<a href='"+b+"' target='_"+a+"'><button class='col-xs-12 col-sm-6 col-md-4 btn btn-default btn-exemplar'>Volltext ("+configlang[a.toUpperCase()]+")</button></a>"});""!=e&&(0<$("#linkresolvercontainer_"+b).parent().length?$("#linkresolvercontainer_"+
b).html("<div>"+configlang.ACCESS+"</div><div class='container-fluid'><div class='row'>"+e+"</div></div>"):$("#linkresolver_"+b).html(e))});$.when($.ajax({url:"/ajax/searchsimularpubs/",method:"POST",data:{ppn:a},dataType:"JSON"})).then(function(a){0<a.hitspack?$("#simularpubscontent_"+b).html(a.results):$("#simularpubscontent_"+b).html("<div class='col-xs-12'><div class='well'>"+configlang.NOSIMULARHITS+"</div></div>")})};
...
$.open_fullview=function(a,b,c,e){void 0===b&&(b=results.ppnlist);void 0===e&&(e=a);void 0===c&&(c="main");"main"!=c&&(b=jQuery.map(b,function(a,b){return a.toString()}));var d=dialogs.length,f=a in favors?configlang.CHECKLISTREMOVE:configlang.CHECKLIST,g=a in favors?"fa fa-star-o":"fa fa-star";e="publications"==c?$("#related_"+e).html():$("#"+e+" div.panel-heading").html();var h=new BootstrapDialog({message:$('<div id="fullbody_'+d+'"></div>').load("/ajax/fullview/"+a+"/"+d),title:e,size:BootstrapDialog.SIZE_WIDE,
...
favormails[a]=$("#mail_"+d).wrap("<p/>").parent().html(),$.user_message(configlang.CHECKLISTSUCCESS,"success"),b.find(".text").text(configlang.CHECKLISTREMOVE),b.find(".fa").removeClass("fa-star"),b.find(".fa").addClass("fa-star-o"));$.update_favors()}}],onhidden:function(){dialogs.pop()},onshown:function(){dialogs.push(h);$.opened_fullview(a,d)}});h.realize();h.getModalHeader().addClass("preview-header");h.setData("ppn",a);h.setData("name","fullview_"+d);h.open()};
...
mailfrom=$("#mailfrom").val();mailto=$("#mailto").val();msg=$("#msg").val();if(""==mailto)return $.user_message(configlang.REQUIREDFIELD,"warning"),$("#mailto").focus(),-1;$.post("/ajax/mailto",{ppnlist:JSON.stringify(a),username:JSON.stringify(c),mailfrom:b,mailto:mailto,msg:JSON.stringify(msg),fullbody:JSON.stringify(favormails)},function(a){"0"<=a&&($.user_message(configlang.MAILSUCCESS,"success"),d.close())})}}],onhidden:function(){dialogs.pop()},onshown:function(){dialogs.push(e);"1"==config.devmode&&
($("#username").val(config.devusername),$("#mailfrom").val(config.devusermail),$("#mailto").val(config.devusermail),$("#msg").val(config.devusermailtext));$("#username").focus()}});e.realize();e.getModalHeader().addClass("preview-header");e.setData("name","mail");e.open()}};
...
void 0==f&&(f=$("#mailto").attr("value"));if(void 0==f)return $.user_message("Die MailTo-Adresse konnte nicht ermittelt werden!","warning"),-1;var g=$("input[name=mailfrom]:checked").val();void 0==g&&(g=$("#mailfrom").attr("value"));if(void 0==g)return $.user_message("Die MailFrom-Adresse konnte nicht ermittelt werden!","warning"),-1;if(""==usrconfig)$.login("LOGINFIRST",function(){var l={},k=$("input[name=volume]").val();void 0!=k&&(l.volume=k);k=$("#mail_"+(dialogs.length-2)).wrap("<p/>").parent().html();
$.post("/ajax/mailorderto",{exemplar:JSON.stringify(c),fullbody:JSON.stringify(k),ppn:a,mailto:f,mailfrom:g,userinput:JSON.stringify(l)},function(a){0>a.status?$.user_message(configlang.PROCESSSTOPPED+"<br />"+a.error,"warning"):(e="success",d=configlang.ORDERSUCCESS,b.close())},"json")});else{var m={},k=$("input[name=volume]").val();void 0!=k&&(m.volume=k);k=$("#mail_"+(dialogs.length-2)).wrap("<p/>").parent().html();$.post("/ajax/mailorderto",{exemplar:JSON.stringify(c),fullbody:JSON.stringify(k),
...
$.detectIE=function(a){a=window.navigator.userAgent;var b=a.indexOf("MSIE ");if(0<b)return parseInt(a.substring(b+5,a.indexOf(".",b)),10);if(0<a.indexOf("Trident/"))return b=a.indexOf("rv:"),parseInt(a.substring(b+3,a.indexOf(".",b)),10);b=a.indexOf("Edge/");return 0<b?parseInt(a.substring(b+5,a.indexOf(".",b)),10):!1};
...
path:"/"});"customer"==$(this).attr("type")?$("#activetheme").attr("href",urlport+window.location.host+"/assets/css/"+$(this).attr("rel")+".css"):"system"==$(this).attr("type")?$("#activetheme").attr("href",urlport+window.location.host+"/systemassets/lukida/css/"+$(this).attr("rel")+".css"):"bootstrap"==$(this).attr("type")&&$("#activetheme").attr("href",urlport+window.location.host+"/systemassets/bootstrap_3.3.6/css/"+$(this).attr("rel")+".min.css");$("#theme li").removeClass("active");$("#theme [rel='"+
$(this).attr("rel")+"']").parent().addClass("active");$.focus_first()});$(window).scroll(function(){Scroll&&$(window).scrollTop()+500>=$(document).height()-$(window).height()&&$.search_next_pack(function(){})});window.scrollTo(0,0);$(".typeahead").typeahead({ajax:"/ajax/getwords",onSelect:function(a){$.new_search("init",a.value)}});window.onpopstate=function(a){$.close_dialogs();$("#facets").fadeOut("fast").addClass("collapse");$("#results").addClass("collapse");$("#resultsonly").html("");$("#loading").fadeOut("fast").addClass("collapse");
null!=a.state&&0<Object.keys(a.state).length?$.facet_search(a.state.text,a.state.facets):($("#start").fadeIn("fast").removeClass("collapse"),$("#searchtext_xs").val(""),$("#searchtext_md").val(""))};$(".yearstart.inlineedit").inlineEdit($('<input name="temp_start" type="text" maxlength="4" size="4" />'),$("#hiddenyearstart"));$(".yearend.inlineedit").inlineEdit($('<input name="temp_end" type="text" maxlength="4" size="4" />'),$("#hiddenyearend"));$("#hiddenyearstart").change(function(){facetvals.yearminstart=
