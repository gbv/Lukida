var Scroll=!1,Height=$(document).height(),title="",user,pw,status,username,mailfrom,mailto,msg,config,usrconfig="",configlang,xtitle=document.title,slider,slider2,dialogs=[],waitdialog,messid=0,appstate={first:!0,pushurl:""!=location.pathname?!1:!0},results={hits:0,hitspack:0,hitscurr:0,qtime:"",ppnlist:{}},searchvals={text:"",last:"","package":1,inittext:"",initfacets:{},search:""},facetvals={datapool:"",yearrange:"",yearmin:"",yearmax:"",yearminstart:"",yearmaxstart:"",typ:"total",formats:{}},statevals=
{ppnlist:[],language:"",layout:"",theme:""},favors={},favormails={},favorprint={},idleTimer,time2warn=1080,time2kill=120,timer=!1,urlport=location.protocol+"//";$.cookie("theme")&&$("[rel='"+$.cookie("theme")+"']").parent().addClass("active");$.is_Empty=function(a){if(null==a)return!0;if(0<a.length)return!1;if(0===a.length||"object"!==typeof a)return!0;for(var b in a)if(hasOwnProperty.call(a,b))return!1;return!0};
$.load_config=function(){$.post("/ajax/config",{screen:JSON.stringify({Width:screen.width,Height:screen.height}),platform:JSON.stringify(platform)},function(a){config=a;$.set_language(config.language,!0);time2warn=60*config.time2warn;time2kill=60*config.time2kill;0==!usrconfig&&(timer=!0,$.startTimer());$.is_Empty(searchvals.initfacets)?facetvals.datapool=config.datapool:(facetvals=searchvals.initfacets,searchvals.initfacets={});$(".facetpool").parents(".dropdown").find(".btn").val(facetvals.datapool);
...
$.user_message=function(a,b,d){void 0==d&&(d=3E3);var c=0<dialogs.length?dialogs[dialogs.length-1].getData("name"):0<results.hits?"results":"statusbar",e="success"==b?"panel-success":"panel-danger";messid++;$("#"+c+"_messagebar").append($("<div id='message"+messid+"' class='alert-message "+b+"' data-alert></div>"));$("#message"+messid).hide().html("<div class='panel "+e+"'><div class='panel-heading'>"+a+"</div></div>").fadeIn("fast");$("#message"+messid).delay(d).fadeOut("slow",function(){$(this).remove()})};
$.focus_first=function(){768<=$(window).width()?$('[type="text"]:input:enabled:visible:first').focus():$("button:enabled:visible:first").focus()};
$.set_facet=function(a,b,d){"datapool"==a&&(facetvals={datapool:d,yearrange:"",yearmin:"",yearmax:"",yearminstart:"",yearmaxstart:"",typ:"total",formats:{}},$.new_search("init",searchvals.text));"year"==a&&($(".num_TYPTOTAL").html(""),$(".num_TYPPRINT").html(""),$(".num_TYPELECTR").html(""),$('input[name="facetonline"][value=total]').prop("checked",!0),facetvals.typ="total",facetvals.formats={},$(".numformat").html(""),$("input.facetformat:checkbox").attr("checked",!1),$.new_search("facet_year"));
"typ"==a&&(facetvals[a]=b,facetvals.formats={},$(".numformat").html(""),$("input.facetformat:checkbox").attr("checked",!1),$.new_search("facet_typ"));"format"==a&&(d?(facetvals.formats[b]=d,$.new_search("facet_format_checked")):(delete facetvals.formats[b],$.new_search("facet_format_unchecked")))};$.link_search=function(a,b){""!=b&&(""!=a&&(b="id"!=a?a+"('"+b+"')":a+"("+b+")"),$.close_dialogs(),$.new_search("link",b))};
$.push_state=function(){if(appstate.pushurl||appstate.first){var a=$.is_Empty(statevals)?{facetvals:facetvals,statevals:{language:config.language,theme:config.theme,layout:config.layout,ppnlist:{}}}:{facetvals:facetvals,statevals:{language:config.language,theme:config.theme,layout:config.layout,ppnlist:statevals.ppnlist}},b=searchvals.text.replace(/"/g,"'").replace(/\*/g,"{star}").replace(/\//g,"{slash}").replace(/</g,"{st}").replace(/>/g,"{gt}").replace(/:/g,"{colon}"),b=urlport+window.location.host+
"/"+encodeURIComponent(b),b=b+("/"+window.btoa(unescape(encodeURIComponent(JSON.stringify(a)))));appstate.first?(window.history.replaceState({text:searchvals.text,facets:facetvals,states:statevals},searchvals.text.substring(0,20)+" | "+xtitle,b),appstate.first=!1):window.history.pushState({text:searchvals.text,facets:facetvals,states:statevals},searchvals.text.substring(0,20)+" | "+xtitle,b)}};$.navbar_search=function(a){a=$(a).val();$.new_search("init",a)};
$.new_search=function(a,b){if("facet_"!=a.substr(0,6).toLowerCase()){searchvals.last=searchvals.text;searchvals.text=void 0==b||""==b?"*":b.trim();if("cmd:"==searchvals.text.substr(0,4).toLowerCase()){$.command(searchvals.text.substr(4));$("#searchtext_xs").val(searchvals.last);$("#searchtext_md").val(searchvals.last);searchvals.text=searchvals.last;$.focus_first();return}$("#searchtext_xs").val(b);$("#searchtext_md").val(b);"initfacets"!=a&&(facetvals.yearrange="",facetvals.yearmin="",facetvals.yearmax=
"",facetvals.yearminstart="",facetvals.yearmaxstart="",facetvals.typ="total",facetvals.formats={},$("#hiddenyearstart").val(""),$("#hiddenyearend").val(""))}Scroll=!1;searchvals["package"]=1;searchvals.search=searchvals.text;$.open_wait();return $.when($.search_results(a)).then(function(){0==results.hits&&$.user_message(configlang.NOHITS,"warning");$.close_wait();document.title=searchvals.text+" | "+xtitle;$.push_state();appstate.pushurl=!0;$.focus_first()})};
$.search_next_pack=function(a){Scroll=!1;$("#loading").removeClass("hideLoading").addClass("displayLoading");searchvals["package"]++;$.when($.search_results("adddata")).then(function(){$("#loading").removeClass("displayLoading").addClass("hideLoading");a()})};
...
$.opened_fullview=function(a,b){$.when($.ajax({url:"/ajax/exportlink/",method:"POST",data:{ppnlist:JSON.stringify(a),format:"zotero"},dataType:"JSON"})).then(function(a){$.each(a,function(a,b){$(".Z3988").attr("title",b)})});0<$("#linkresolver_"+b).length||0<$("#linkresolvercontainer_"+b).length?(0<$("#linkresolvercontainer_"+b).length?$("#linkresolvercontainer_"+b).html("<div class='container-fluid'><div class='row'><div class='col-xs-12 col-sm-6 col-md-4'><div class='outercircle'></div><div class='innercircle'></div></div></div></div>"):
$("#linkresolver_"+b).html("<div class='col-xs-12 col-sm-6 col-md-4'><div class='outercircle'></div><div class='innercircle'></div></div>"),$.when($.ajax({url:"/ajax/linkresolver/",method:"POST",data:{ppn:a},dataType:"JSON"})).then(function(a){var c="";$.each(a,function(a,b){c+="<button onclick='$.openLink(\""+b+"\")' class='col-xs-12 col-sm-6 col-md-4 btn btn-default btn-exemplar'>Volltext ("+configlang[a.toUpperCase()]+")</button>"});""!=c?0<$("#linkresolvercontainer_"+b).length?$("#linkresolvercontainer_"+
b).html("<div>"+configlang.ACCESS+"</div><div class='container-fluid'><div class='row'>"+c+"</div></div>"):$("#linkresolver_"+b).html(c):($("#linkresolvercontainer_"+b).html(""),$("#linkresolver_"+b).html(""))})):($("#linkresolvercontainer_"+b).html(""),$("#linkresolver_"+b).html(""));$.when($.ajax({url:"/ajax/searchsimularpubs/",method:"POST",data:{ppn:a},dataType:"JSON"})).then(function(a){0<a.hitspack?$("#simularpubscontent_"+b).html(a.results):$("#simularpubscontent_"+b).html("<div class='col-xs-12'><div class='well'>"+
configlang.NOSIMULARHITS+"</div></div>")})};$.correctCover=function(a,b){a.onerror="";$(a).replaceWith("<span class='gbvicon'>"+b+"</span>")};
...
$(".datapool").change(function(){$.set_facet("datapool",$(this).val(),this.checked)});var d="";$.each(a.formats,function(a,b){d+="<div class='checkbox'><label><input class='facetformat' id='"+a.replace(/ /g,"")+"' value='"+a+"' type='checkbox'><span class='lbl_"+a.replace(/ /g,"")+"'>"+configlang[a.replace(/ /g,"").toUpperCase()]+"</span></label><span class='num_"+a.replace(/ /g,"")+" numformat badge counter pull-right'>"+b+"</span></div>"});$("#formats").html(d);$(".facetformat").change(function(){$.set_facet("format",
...
$.open_fullview=function(a,b,d,c){void 0===b&&(b=results.ppnlist);void 0===c&&(c=a);void 0===d&&(d="main");"main"!=d&&(b=jQuery.map(b,function(a,b){return a.toString()}));var e=dialogs.length,f=a in favors?configlang.CHECKLISTREMOVE:configlang.CHECKLIST,g=a in favors?"fa fa-star-o":"fa fa-star",k="publications"==d?$("#related_"+c).html():$("#"+c+" div.panel-heading").html(),h=new BootstrapDialog({message:$('<div id="fullbody_'+e+'"><div class="outercircle"></div><div class="innercircle"></div>').load("/ajax/fullview/"+
a+"/"+e,function(){config.button_checklist&&h.getButton("checklist").prop("disabled",!1);config.button_export&&h.getButton("export").prop("disabled",!1);config.button_mail&&h.getButton("mail").prop("disabled",!1);config.button_print&&h.getButton("print").prop("disabled",!1)}),title:k,size:BootstrapDialog.SIZE_WIDE,closable:!0,buttons:[{id:"prevbtn",icon:"glyphicon glyphicon-chevron-left",cssClass:"btn-default fullview-button-left",action:function(c){0<b.indexOf(a)?(a=b[b.indexOf(a)-1],$("#fullbody_"+
e).slideUp("fast","swing",function(){$("#fullbody_"+e).load("/ajax/fullview/"+a+"/"+e,function(){"publications"==d?c.getModalHeader().find(".bootstrap-dialog-title").html($("#related_"+a).html()):c.getModalHeader().find(".bootstrap-dialog-title").html($("#"+a+" div.panel-heading").html());$("#fullbody_"+e).slideDown("fast");var b=c.getModalFooter().find(".checklist");a in favors?(b.find(".text").text(configlang.CHECKLISTREMOVE),b.find(".fa").removeClass("fa-star"),b.find(".fa").addClass("fa-star-o")):
(b.find(".text").text(configlang.CHECKLIST),b.find(".fa").removeClass("fa-star-o"),b.find(".fa").addClass("fa-star"));$.opened_fullview(a,e)})})):$.user_message(configlang.FIRSTOBJECT,"warning")}},{id:"nextbtn",icon:"glyphicon glyphicon-chevron-right",cssClass:"btn-default fullview-button-left",action:function(c){"main"==d&&b.indexOf(a)<results.hits-1&&b.indexOf(a)==results.hitscurr-1?$.search_next_pack(function(){b=results.ppnlist;a=b[b.indexOf(a)+1];$("#fullbody_"+e).slideUp("fast","swing",function(){$("#fullbody_"+
e).load("/ajax/fullview/"+a+"/"+e,function(){"publications"==d?c.getModalHeader().find(".bootstrap-dialog-title").html($("#related_"+a).html()):c.getModalHeader().find(".bootstrap-dialog-title").html($("#"+a+" div.panel-heading").html());$("#fullbody_"+e).slideDown("fast");var b=c.getModalFooter().find(".checklist");a in favors?(b.find(".text").text(configlang.CHECKLISTREMOVE),b.find(".fa").removeClass("fa-star"),b.find(".fa").addClass("fa-star-o")):(b.find(".text").text(configlang.CHECKLIST),b.find(".fa").removeClass("fa-star-o"),
b.find(".fa").addClass("fa-star"));$.opened_fullview(a,e)})})}):b.indexOf(a)<b.length-1?(a=b[b.indexOf(a)+1],$("#fullbody_"+e).slideUp("fast","swing",function(){$("#fullbody_"+e).load("/ajax/fullview/"+a+"/"+e,function(){"publications"==d?c.getModalHeader().find(".bootstrap-dialog-title").html($("#related_"+a).html()):c.getModalHeader().find(".bootstrap-dialog-title").html($("#"+a+" div.panel-heading").html());$("#fullbody_"+e).slideDown("fast");var b=c.getModalFooter().find(".checklist");a in favors?
(b.find(".text").text(configlang.CHECKLISTREMOVE),b.find(".fa").removeClass("fa-star"),b.find(".fa").addClass("fa-star-o")):(b.find(".text").text(configlang.CHECKLIST),b.find(".fa").removeClass("fa-star-o"),b.find(".fa").addClass("fa-star"));$.opened_fullview(a,e)})})):$.user_message(configlang.LASTOBJECT,"warning")}},{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(a){a.close();$.focus_first()}},{id:"print",icon:"fa fa-print",label:" "+configlang.PRINT,action:function(a){a=
window.open();a.document.write("<html><head><title>"+configlang.PRINT+"</title></head><body>");a.document.write($("#mail_"+e).wrap("<p/>").parent().html());a.document.write("</body></html");a.document.close();a.focus();a.print();a.close()}},{id:"mail",icon:"glyphicon glyphicon-envelope",label:" "+configlang.MAIL,action:function(b){favormails[a]=$("#mail_"+e).wrap("<p/>").parent().html();""==usrconfig?$.login("LOGINFIRST",function(){""!=usrconfig&&$.open_mail([a])}):$.open_mail([a])}},{id:"export",
icon:"glyphicon glyphicon-export",label:" "+configlang.EXPORT,action:function(b){$.open_export([a])}},{id:"checklist",icon:g,cssClass:"btn-default checklist",label:' <span class="text">'+f+"</span>",action:function(b){b=$(this);a in favors?(delete favors[a],delete favormails[a],delete favorprint[a],$.user_message(configlang.CHECKLISTREMOVESUCCESS,"success"),b.find(".text").text(configlang.CHECKLIST),b.find(".fa").removeClass("fa-star-o"),b.find(".fa").addClass("fa-star")):(favors[a]=$("#"+a).clone().removeClass("medium col-xs-12 col-sm-6 col-md-4 col-lg-3",
"col-md-6").addClass("merk col-md-6"),$(favors[a]).find("a").attr("href","javascript:$.open_fullview('"+a+"',Object.keys(favors));"),favormails[a]=$("#mail_"+e).wrap("<p/>").parent().html(),favorprint[a]=$("#mail_"+e).wrap("<p/>").parent().html(),$.user_message(configlang.CHECKLISTSUCCESS,"success"),b.find(".text").text(configlang.CHECKLISTREMOVE),b.find(".fa").removeClass("fa-star"),b.find(".fa").addClass("fa-star-o"));$.update_favors()}}],onhidden:function(){dialogs.pop();statevals.ppnlist.pop();
$.push_state()},onshown:function(){dialogs.push(h);$.is_Empty(statevals)&&(statevals={});$.is_Empty(statevals.ppnlist)&&(statevals.ppnlist=[]);statevals.ppnlist.push({ppn:a,ppnlist:b,typ:d,parentppn:c});$.push_state();$.opened_fullview(a,e)}});h.realize();h.getModalHeader().addClass("preview-header");config.button_checklist?h.getButton("checklist").prop("disabled",!0):h.getButton("checklist").hide();config.button_export?h.getButton("export").prop("disabled",!0):h.getButton("export").hide();config.button_mail?
h.getButton("mail").prop("disabled",!0):h.getButton("mail").hide();config.button_print?h.getButton("print").prop("disabled",!0):h.getButton("print").hide();h.setData("ppn",a);h.setData("name","fullview_"+e);h.open()};
$.open_assistant=function(){var a=dialogs.length,b=new BootstrapDialog({message:$('<div id="assistant_'+a+'"></div>').load("/ajax/assistant/"+a,function(){}),title:configlang.ASSISTANT,size:BootstrapDialog.SIZE_WIDE,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(a){a.close();$.focus_first()}},{icon:"fa fa-eraser",label:" "+configlang.CLEAR,action:function(b){$("#title_"+a+" textarea").val("");$("#author_"+a+" textarea").val("");$("#subject_"+a+
" textarea").val("");$("#class_"+a+" textarea").val("");$("#contents_"+a+" textarea").val("");$("#year_"+a+" textarea").val("");$("#publisher_"+a+" textarea").val("");$("#isn_"+a+" textarea").val("");$("#series_"+a+" textarea").val("");$("#id_"+a+" textarea").val("");$.focus_first()}},{icon:"glyphicon glyphicon-search",label:" "+configlang.STARTSEARCH,cssClass:"fullview-button-color",action:function(b){var c=b="",c=$.cleanArray($("#title_"+a+" textarea").val().split("\n"));0<c.length&&(b+=' title("'+
c.join('","')+'")');c=$.cleanArray($("#author_"+a+" textarea").val().split("\n"));0<c.length&&(b+=' author("'+c.join('","')+'")');c=$.cleanArray($("#subject_"+a+" textarea").val().split("\n"));0<c.length&&(b+=' subject("'+c.join('","')+'")');c=$.cleanArray($("#class_"+a+" textarea").val().split("\n"));0<c.length&&(b+=' class("'+c.join('","')+'")');c=$.cleanArray($("#contents_"+a+" textarea").val().split("\n"));0<c.length&&(b+=' contents("'+c.join('","')+'")');c=$.cleanArray($("#year_"+a+" textarea").val().split("\n"));
0<c.length&&(b+=' year("'+c.join('","')+'")');c=$.cleanArray($("#publisher_"+a+" textarea").val().split("\n"));0<c.length&&(b+=' publisher("'+c.join('","')+'")');c=$.cleanArray($("#isn_"+a+" textarea").val().split("\n"));0<c.length&&(b+=' isn("'+c.join('","')+'")');c=$.cleanArray($("#series_"+a+" textarea").val().split("\n"));0<c.length&&(b+=' series("'+c.join('","')+'")');c=$.cleanArray($("#id_"+a+" textarea").val().split("\n"));0<c.length&&(b+=' id("'+c.join('","')+'")');$.when($.new_search("init",
b)).then(function(){0==results.hits?$("#assistantcount").html("<h4>"+configlang.ASSISTANTNO+"</h4>"):$("#assistantcount").html("<h4>"+$.format_hits(results.hits)+" "+configlang.ASSISTANTYES+"</h4>")})}}],onhidden:function(){dialogs.pop()},onshown:function(){dialogs.push(b)}});b.realize();b.getModalHeader().addClass("preview-header");b.setData("name","assistant_"+a);b.open()};
$.update_favors=function(){$("#favors").html("");if(0<Object.keys(favors).length){var a=0;$.each(favors,function(b,d){a++;1==a%2&&$("#favors").append("<div class='row'>");$("#favors").append(d);0==a%2&&$("#favors").append("</div>")});1==a%2&&$("#favors").append("</div>")}else $("#favors").append("<div class='col-xs-12'>"+configlang.CHECKLISTEMPTY+"</div>")};
$.open_favors=function(){var a="",b="",d=new BootstrapDialog({message:'<div class="row" id="favors"></div><div id="favor_messagebar"></div>',title:"Merkliste "+config.softwarename,closable:!0,size:BootstrapDialog.SIZE_WIDE,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(a){a.close()}},{id:"print",icon:"fa fa-print",label:" "+configlang.PRINT,action:function(a){if(0<Object.keys(favorprint).length){var b=window.open();b.document.write("<html><head><title>"+configlang.PRINT+
"</title></head><body>");$.each(favorprint,function(a,c){b.document.write(c);b.document.write("<hr>")});b.document.write("</body></html");b.document.close();b.focus();b.print();b.close()}}},{id:"mail",icon:"glyphicon glyphicon-envelope",label:" "+configlang.MAIL,action:function(a){""==usrconfig?$.login("LOGINFIRST",function(){""!=usrconfig&&$.open_mail(Object.keys(favors))}):$.open_mail(Object.keys(favors))}},{id:"export",icon:"glyphicon glyphicon-export",label:" "+configlang.EXPORT,action:function(a){$.open_export(Object.keys(favors))}},
{icon:"fa fa-eraser",label:" "+configlang.CHECKLISTCLEAR,action:function(c){favors={};favorprint={};favormail={};a="success";b=configlang.CHECKLISTCLEARSUCCESS;c.close()}}],onhidden:function(){dialogs.pop();""!=a&&$.user_message(b,a);$.focus_first()},onshown:function(){dialogs.push(d);$.update_favors()}});d.realize();d.getModalHeader().addClass("preview-header");config.button_export||d.getButton("export").hide();config.button_mail||d.getButton("mail").hide();config.button_print||d.getButton("print").hide();
d.setData("name","favors");d.open()};
$.open_mail=function(a){if(""!=usrconfig){void 0!=usrconfig.email&&""!=usrconfig.email||$.user_message(configlang.PROVIDEMAIL,"warning");var b=usrconfig.email,d=(usrconfig.firstname+" "+usrconfig.lastname).trim(),c=new BootstrapDialog({message:'<div class="form-horizontal"><div class="form-group"><label for="mailto" class="col-sm-3 control-label">'+configlang.MAILRECIPIENT+'</label><div class="col-sm-9"><input type="mail" class="form-control" id="mailto" placeholder="'+configlang.MAILRECIPIENT+'"></div></div><div class="form-group"><label for="msg" class="col-sm-3 control-label">'+
configlang.YOURMESSAGE+'</label><div class="col-sm-9"><textarea class="form-control" rows="5" id="msg" placeholder="'+configlang.YOURMESSAGE+'"></textarea></div></div><div id="mail_messagebar"></div></div>',title:configlang.SENDMAIL,closable:!1,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(a){status=-1;a.close()}},{icon:"glyphicon glyphicon-log-in",label:" "+configlang.SENDMAIL,cssClass:"fullview-button-color",action:function(c){username=$("#username").val();
mailfrom=$("#mailfrom").val();mailto=$("#mailto").val();msg=$("#msg").val();if(""==mailto)return $.user_message(configlang.REQUIREDFIELD,"warning"),$("#mailto").focus(),-1;if(!$.validateEmail(mailto))return $.user_message(configlang.MAILWRONG,"warning"),$("#mailto").focus(),-1;$.post("/ajax/mailto",{ppnlist:JSON.stringify(a),username:JSON.stringify(d),mailfrom:b,mailto:mailto,msg:JSON.stringify(msg),fullbody:JSON.stringify(favormails)},function(a){"0"<=a&&($.user_message(configlang.MAILSUCCESS,"success"),
c.close())})}}],onhidden:function(){dialogs.pop()},onshown:function(){dialogs.push(c);"1"==config.devmode&&($("#username").val(config.devusername),$("#mailfrom").val(config.devusermail),$("#mailto").val(config.devusermail),$("#msg").val(config.devusermailtext));$("#username").focus()}});c.realize();c.getModalHeader().addClass("preview-header");c.setData("name","mail");c.open()}};
...
closable:!1,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(a){status=-1;a.close()}},{icon:"glyphicon glyphicon-export",label:" "+configlang.EXPORT,cssClass:"fullview-button-color",action:function(b){var d=$("input[name=export]:checked").val();"refworks"==d?$.post("/ajax/exportlink",{ppnlist:JSON.stringify(a),format:d},function(a){""!=a&&$.each(a,function(a,b){window.open(b,"_blank")})},"json"):$.each(a,function(a,b){window.open(urlport+window.location.host+
"/exportfile/"+b+"/"+d,"_blank")});b.close()}}],onhidden:function(){dialogs.pop()},onshown:function(){dialogs.push(exportdialog)}});exportdialog.realize();exportdialog.getModalHeader().addClass("preview-header");exportdialog.setData("name","export");exportdialog.open()};
...
$.shelve=function(a,b,d){var c=b="",e;for(e in d)"action"!=e&&"typ"!=e&&"id"!=e&&""!=d[e]&&(""==c&&(c=e.replace(/\d/g,"")),""!=c&&c!=e.replace(/\d/g,"")&&(c=e.replace(/\d/g,""),b+="<br />"),b+=d[e].replace(/%lt%/g,"&lt;").replace(/%gt%/g,"&gt;")+"<br />");b+='<br /><div id="shelve_messagebar"></div>';d=$("#"+a+" div.panel-heading").html();void 0==d&&(d=$("#related_"+a).html());var f=new BootstrapDialog({message:b,title:d,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,
...
$.reservation=function(a,b,d){var c=b="",e;for(e in d)"action"!=e&&"typ"!=e&&"id"!=e&&""!=d[e]&&(""==c&&(c=e.replace(/\d/g,"")),""!=c&&c!=e.replace(/\d/g,"")&&(c=e.replace(/\d/g,""),b+="<br />"),b+=d[e].replace(/%lt%/g,"&lt;").replace(/%gt%/g,"&gt;")+"<br />");b+='<br /><div id="reservation_messagebar"></div>';e=$("#"+a+" div.panel-heading").html();void 0==e&&(e=$("#related_"+a).html());var f="",g="",k=new BootstrapDialog({message:b,title:e,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",
label:" "+configlang.CANCEL,action:function(a){a.close();$.focus_first()}},{icon:"fa fa-map-marker",label:" "+configlang.RESERVATION,cssClass:"fullview-button-color",action:function(a){""==usrconfig?$.login("LOGINFIRST",function(){$.post("/ajax/request",{uri:d.id},function(b){0>b.status?(f="warning",g=configlang.PROCESSSTOPPED+"<br />"+b.error):(f="success",g=configlang.RESERVATIONSUCCESS);a.close()},"json")}):$.post("/ajax/request",{uri:d.id},function(b){0>b.status?(f="warning",g=configlang.PROCESSSTOPPED+
"<br />"+b.error):(f="success",g=configlang.RESERVATIONSUCCESS);a.close()},"json")}}],onhidden:function(){dialogs.pop();""!=f&&$.user_message(g,f)},onshown:function(){dialogs.push(k)}});k.realize();k.getModalHeader().addClass("preview-header");k.setData("name","reservation");k.open()};
$.order=function(a,b,d){if(void 0!=d.id){var c=b="",e;for(e in d)"action"!=e&&"typ"!=e&&"id"!=e&&""!=d[e]&&(""==c&&(c=e.replace(/\d/g,"")),""!=c&&c!=e.replace(/\d/g,"")&&(c=e.replace(/\d/g,""),b+="<br />"),b+=d[e].replace(/%lt%/g,"&lt;").replace(/%gt%/g,"&gt;")+"<br />");b+='<br /><div id="order_messagebar"></div>';e=$("#"+a+" div.panel-heading").html();void 0==e&&(e=$("#related_"+a).html());var f="",g="",k=new BootstrapDialog({message:b,title:e,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",
label:" "+configlang.CANCEL,action:function(a){a.close();$.focus_first()}},{icon:"fa fa-map-marker",label:" "+configlang.ORDER,cssClass:"fullview-button-color",action:function(a){""==usrconfig?$.login("LOGINFIRST",function(){$.post("/ajax/request",{uri:d.id},function(b){0>b.status?(f="warning",g=configlang.PROCESSSTOPPED+"<br />"+b.error):(f="success",g=configlang.ORDERSUCCESS);a.close()},"json")}):$.post("/ajax/request",{uri:d.id},function(b){0>b.status?(f="warning",g=configlang.PROCESSSTOPPED+"<br />"+
b.error):(f="success",g=configlang.ORDERSUCCESS);a.close()},"json")}}],onhidden:function(){dialogs.pop();""!=f&&$.user_message(g,f)},onshown:function(){dialogs.push(k)}});k.realize();k.getModalHeader().addClass("preview-header");k.setData("name","order");k.open()}};
$.mailorder=function(a,b,d){var c="",e="",f=$("#"+a+" div.panel-heading").html();void 0==f&&(f=$("#related_"+a).html());var g=new BootstrapDialog({message:$('<div id="mailorderviewbody"></div>').load("/ajax/mailorderview/"+a+"/"+b),title:f,closable:!0,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(a){a.close();$.focus_first()}},{icon:"fa fa-map-marker",label:" "+configlang.ORDER,cssClass:"fullview-button-color",action:function(b){var f=$("input[name=mailto]:checked").val();
void 0==f&&(f=$("#mailto").attr("value"));if(void 0==f)return $.user_message("Die MailTo-Adresse konnte nicht ermittelt werden!","warning"),-1;var g=$("input[name=mailfrom]:checked").val();void 0==g&&(g=$("#mailfrom").attr("value"));if(void 0==g)return $.user_message("Die MailFrom-Adresse konnte nicht ermittelt werden!","warning"),-1;if(""==usrconfig)$.login("LOGINFIRST",function(){var n={},l=$("input[name=volume]").val();void 0!=l&&(n.volume=l);l=$("#mail_"+(dialogs.length-2)).wrap("<p/>").parent().html();
$.post("/ajax/mailorderto",{exemplar:JSON.stringify(d),fullbody:JSON.stringify(l),ppn:a,mailto:f,mailfrom:g,userinput:JSON.stringify(n)},function(a){0>a.status?$.user_message(configlang.PROCESSSTOPPED+"<br />"+a.error,"warning"):(c="success",e=configlang.ORDERSUCCESS,b.close())},"json")});else{var m={},l=$("input[name=volume]").val();void 0!=l&&(m.volume=l);l=$("#mail_"+(dialogs.length-2)).wrap("<p/>").parent().html();$.post("/ajax/mailorderto",{exemplar:JSON.stringify(d),fullbody:JSON.stringify(l),
ppn:a,mailto:f,mailfrom:g,userinput:JSON.stringify(m)},function(a){0>a.status?$.user_message(configlang.PROCESSSTOPPED+"<br />"+a.error,"warning"):(c="success",e=configlang.ORDERSUCCESS,b.close())},"json")}}}],onhidden:function(){dialogs.pop();""!=c&&$.user_message(e,c)},onshown:function(){dialogs.push(g)}});g.realize();g.getModalHeader().addClass("preview-header");g.setData("name","mailorder");g.open()};
...
$.whenUserIdle=function(){$.user_message(configlang.IDLEHAPPENED,"warning");$.post("/ajax/logout",function(a){$(".nologinarea").removeClass("hide");$(".loginarea").addClass("hide")},"json");$.reset_session()};$.startCountdown=function(a,b){var d=a;setInterval(function(){b.html(d);0>--d&&(d=a)},1E3)};
$.fn.inlineEdit=function(a,b){$.detectIE()||$(this).click(function(){var d=$(this);d.hide();d.after(a);a.focus();"temp_start"==a.attr("name")&&(""!=facetvals.yearminstart?a.val(facetvals.yearminstart):a.val(facetvals.yearmin));"temp_end"==a.attr("name")&&(""!=facetvals.yearmaxstart?a.val(facetvals.yearmaxstart):a.val(facetvals.yearmax));a.keypress(function(a){13!=a.keypress&&13!=a.keyCode||$.focus_first()});a.blur(function(){""!=$(this).val()&&($(this).val()<facetvals.yearmin||$(this).val()>facetvals.yearmax||
"temp_start"==$(this).attr("name")&&$(this).val()>facetvals.yearmaxstart||"temp_end"==$(this).attr("name")&&$(this).val()<facetvals.yearminstart?(window.scrollTo(0,0),$.user_message(configlang.CORRECTPERIOD.replace("{PERIOD}",facetvals.yearmin+"-"+facetvals.yearmax)),$(this).val("")):(b.val($(this).val()).change(),d.text($(this).val())));$(this).remove();d.show()})})};
...
$.cleanArray=function(a){for(var b=0;b<a.length;b++)void 0==a[b].trim()||""==a[b].trim()?(a.splice(b,1),b--):a[b]=a[b].trim();return a};$.openLink=function(a){$.post("/ajax/statsclient",{typ:"Link",total:!0,link:a});window.open(a,"_blank")};
