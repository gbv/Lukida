...
(b.find(".text").text(configlang.CHECKLIST),b.find(".fa").removeClass("fa-star-o"),b.find(".fa").addClass("fa-star"));$.opened_fullview(a,e)})})}):b.indexOf(a)<b.length-1?(a=b[b.indexOf(a)+1],$("#fullbody_"+e).slideUp("fast","swing",function(){$("#fullbody_"+e).load("/ajax/fullview/"+a+"/"+e,function(){"publications"==d?c.getModalHeader().find(".bootstrap-dialog-title").html($("#related_"+a).html()):c.getModalHeader().find(".bootstrap-dialog-title").html($("#"+a+" div.panel-heading").html());$("#fullbody_"+
e).slideDown("fast");var b=c.getModalFooter().find(".checklist");a in favors?(b.find(".text").text(configlang.CHECKLISTREMOVE),b.find(".fa").removeClass("fa-star"),b.find(".fa").addClass("fa-star-o")):(b.find(".text").text(configlang.CHECKLIST),b.find(".fa").removeClass("fa-star-o"),b.find(".fa").addClass("fa-star"));$.opened_fullview(a,e)})})):$.user_message(configlang.LASTOBJECT,"warning")}},{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(a){a.close();$.focus_first()}},
{id:"print",icon:"fa fa-print",label:" "+configlang.PRINT,action:function(a){$(this).blur();a=window.open();a.document.write("<html><head><title>"+configlang.PRINT+"</title></head><body>");a.document.write($("#mail_"+e).wrap("<p/>").parent().html());a.document.write("</body></html");a.document.close();a.focus();a.print();a.close()}},{id:"mail",icon:"glyphicon glyphicon-envelope",label:" "+configlang.MAIL,action:function(b){$(this).blur();favormails[a]=$("#mail_"+e).wrap("<p/>").parent().html();""==
usrconfig?$.login("LOGINFIRST",function(){""!=usrconfig&&$.open_mail([a])}):$.open_mail([a])}},{id:"export",icon:"glyphicon glyphicon-export",label:" "+configlang.EXPORT,action:function(b){$(this).blur();$.open_export([a])}},{id:"checklist",icon:h,cssClass:"btn-default checklist",label:' <span class="text">'+f+"</span>",action:function(b){$(this).blur();b=$(this);a in favors?(delete favors[a],delete favormails[a],delete favorprint[a],$.user_message(configlang.CHECKLISTREMOVESUCCESS,"success"),b.find(".text").text(configlang.CHECKLIST),
b.find(".fa").removeClass("fa-star-o"),b.find(".fa").addClass("fa-star")):(favors[a]=$("#"+a).clone().removeClass("medium col-xs-12 col-sm-6 col-md-4 col-lg-3","col-md-6").addClass("merk col-md-6"),$(favors[a]).find("a").attr("href","javascript:$.open_fullview('"+a+"',Object.keys(favors));"),favormails[a]=$("#mail_"+e).wrap("<p/>").parent().html(),favorprint[a]=$("#mail_"+e).wrap("<p/>").parent().html(),$.user_message(configlang.CHECKLISTSUCCESS,"success"),b.find(".text").text(configlang.CHECKLISTREMOVE),
b.find(".fa").removeClass("fa-star"),b.find(".fa").addClass("fa-star-o"));$.update_favors()}}],onhidden:function(){dialogs.pop();statevals.ppnlist.pop();$.push_state()},onshown:function(){dialogs.push(g);$.is_Empty(statevals)&&(statevals={});$.is_Empty(statevals.ppnlist)&&(statevals.ppnlist=[]);statevals.ppnlist.push({ppn:a,ppnlist:b,typ:d,parentppn:c});$.push_state()}});g.realize();g.getModalHeader().addClass("preview-header");config.button_checklist?g.getButton("checklist").prop("disabled",!0):
g.getButton("checklist").hide();config.button_export?g.getButton("export").prop("disabled",!0):g.getButton("export").hide();config.button_mail?g.getButton("mail").prop("disabled",!0):g.getButton("mail").hide();config.button_print?g.getButton("print").prop("disabled",!0):g.getButton("print").hide();g.setData("ppn",a);g.setData("name","fullview_"+e);g.open()};
...
configlang.EXPORT,action:function(a){$(this).blur();$.open_export(Object.keys(favors))}},{icon:"fa fa-eraser",label:" "+configlang.CHECKLISTCLEAR,action:function(c){$(this).blur();favors={};favorprint={};favormail={};a="success";b=configlang.CHECKLISTCLEARSUCCESS;c.close()}}],onhidden:function(){dialogs.pop();""!=a&&$.user_message(b,a);$.focus_first()},onshown:function(){dialogs.push(d);$.update_favors()}});d.realize();d.getModalHeader().addClass("preview-header");config.button_export||d.getButton("export").hide();
config.button_mail||d.getButton("mail").hide();config.button_print||d.getButton("print").hide();d.setData("name","favors");d.open()};
...
closable:!1,buttons:[{icon:"glyphicon glyphicon-menu-up",label:" "+configlang.CANCEL,action:function(a){$(this).blur();status=-1;a.close()}},{icon:"glyphicon glyphicon-export",label:" "+configlang.EXPORT,cssClass:"fullview-button-color",action:function(b){$(this).blur();var d=$("input[name=export]:checked").val();"refworks"==d?$.post("/ajax/exportlink",{ppnlist:JSON.stringify(a),format:d},function(a){""!=a&&$.each(a,function(a,b){window.open(b,"_blank")})},"json"):$.each(a,function(a,b){window.open(urlport+
window.location.host+"/exportfile/"+b+"/"+d,"_blank")});b.close()}}],onhidden:function(){dialogs.pop()},onshown:function(){dialogs.push(exportdialog)}});exportdialog.realize();exportdialog.getModalHeader().addClass("preview-header");exportdialog.setData("name","export");exportdialog.open()};
...
"json")}}],onhidden:function(){dialogs.pop();""!=d&&$.user_message(c,d);""!=e&&$.user_message(f,e,1E4);b()},onshown:function(){dialogs.push(h);"1"==config.devmode&&($("#kennung").val(config.devuser),$("#passwort").val(config.devpassword));$("#kennung").focus();$(".keyenter").keypress(function(a){13==a.which&&($(this).blur(),$("#loginbutton").focus().click())});void 0!=a&&""!=a&&$.user_message(configlang[a],"warning")}});h.realize();h.getModalHeader().addClass("preview-header");h.setData("name","login");
h.open()};$.loginonly=function(){$.login("",function(){})};
...
label:" "+configlang.CANCEL,action:function(a){a.close();$.focus_first()}},{icon:"fa fa-map-marker",label:" "+configlang.RESERVATION,cssClass:"fullview-button-color",action:function(a){$(this).blur();""==usrconfig?$.login("LOGINFIRST",function(){$.post("/ajax/request",{uri:d.id},function(b){0>b.status?(f="warning",h=configlang.PROCESSSTOPPED+"<br />"+b.error):(f="success",h=configlang.RESERVATIONSUCCESS);a.close()},"json")}):$.post("/ajax/request",{uri:d.id},function(b){0>b.status?(f="warning",h=
configlang.PROCESSSTOPPED+"<br />"+b.error):(f="success",h=configlang.RESERVATIONSUCCESS);a.close()},"json")}}],onhidden:function(){dialogs.pop();""!=f&&$.user_message(h,f)},onshown:function(){dialogs.push(k)}});k.realize();k.getModalHeader().addClass("preview-header");k.setData("name","reservation");k.open()};
...
"<br />"+b.error):(f="success",h=configlang.ORDERSUCCESS);a.close()},"json")}}],onhidden:function(){dialogs.pop();""!=f&&$.user_message(h,f)},onshown:function(){dialogs.push(k)}});k.realize();k.getModalHeader().addClass("preview-header");k.setData("name","order");k.open()}};
...
$.focus_first()}},{icon:"fa fa-map-marker",label:" "+configlang[h.toUpperCase()],cssClass:"fullview-button-color",action:function(b){$(this).blur();var f=$("input[name=mailto]:checked").val();void 0==f&&(f=$("#mailto").attr("value"));if(void 0==f)return $.user_message("Die MailTo-Adresse konnte nicht ermittelt werden!","warning"),-1;var g=$("input[name=mailfrom]:checked").val();void 0==g&&(g=$("#mailfrom").attr("value"));if(void 0==g)return $.user_message("Die MailFrom-Adresse konnte nicht ermittelt werden!",
"warning"),-1;if(""==usrconfig)$.login("LOGINFIRST",function(){var h={},l=$("input[name=volume]").val();void 0!=l&&(h.volume=l);l=$("#mail_"+(dialogs.length-2)).wrap("<p/>").parent().html();$.post("/ajax/mailorderto",{exemplar:JSON.stringify(d),fullbody:JSON.stringify(l),ppn:a,mailto:f,mailfrom:g,userinput:JSON.stringify(h),mailtyp:k},function(a){0>a.status?$.user_message(configlang.PROCESSSTOPPED+"<br />"+a.error,"warning"):(c="success",e=configlang.ORDERSUCCESS,b.close())},"json")});else{var h=
{},l=$("input[name=volume]").val();void 0!=l&&(h.volume=l);l=$("#mail_"+(dialogs.length-2)).wrap("<p/>").parent().html();$.post("/ajax/mailorderto",{exemplar:JSON.stringify(d),fullbody:JSON.stringify(l),ppn:a,mailto:f,mailfrom:g,userinput:JSON.stringify(h),mailtyp:k},function(a){0>a.status?$.user_message(configlang.PROCESSSTOPPED+"<br />"+a.error,"warning"):(c="success",e=configlang.ORDERSUCCESS,b.close())},"json")}}}],onhidden:function(){dialogs.pop();""!=c&&$.user_message(e,c)},onshown:function(){dialogs.push(g)}});
g.realize();g.getModalHeader().addClass("preview-header");g.setData("name","mailorder");g.open()};$.cancel=function(a){$.post("/ajax/cancel",{uri:a},function(a){0>a.status?$.user_message(configlang.PROCESSSTOPPED+"<br />"+a.error,"warning"):$.user_message(configlang.CANCELSUCCESS,"success")},"json")};
