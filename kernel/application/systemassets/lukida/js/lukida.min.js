...
$.opened_fullview=function(a,b){$.when($.ajax({url:"/ajax/exportlink/",method:"POST",data:{ppnlist:JSON.stringify(a),format:"zotero"},dataType:"JSON"})).then(function(a){$.each(a,function(a,b){$(".Z3988").attr("title",b)})});(0<$("#linkresolver_"+b).parent().length||0<$("#linkresolvercontainer_"+b).parent().length)&&$.when($.ajax({url:"/ajax/linkresolver/",method:"POST",data:{ppn:a},dataType:"JSON"})).then(function(a){var c="";$.each(a,function(a,b){c+="<a href='"+b+"' target='_"+a+"'><button class='col-xs-12 col-sm-6 col-md-4 btn btn-default btn-exemplar'>Volltext ("+
configlang[a.toUpperCase()]+")</button></a>"});""!=c&&(0<$("#linkresolvercontainer_"+b).parent().length?$("#linkresolvercontainer_"+b).html("<div>"+configlang.ACCESS+"</div><div class='container-fluid'><div class='row'>"+c+"</div></div>"):$("#linkresolver_"+b).html(c))});$.when($.ajax({url:"/ajax/searchsimularpubs/",method:"POST",data:{ppn:a},dataType:"JSON"})).then(function(a){0<a.hitspack?$("#simularpubscontent_"+b).html(a.results):$("#simularpubscontent_"+b).html("<div class='col-xs-12'><div class='well'>"+
...
$.push_state()},onshown:function(){dialogs.push(h);$.is_Empty(statevals)&&(statevals={});$.is_Empty(statevals.ppnlist)&&(statevals.ppnlist=[]);statevals.ppnlist.push({ppn:a,ppnlist:b,typ:d,parentppn:c});$.push_state()}});h.realize();h.getModalHeader().addClass("preview-header");config.button_checklist?h.getButton("checklist").prop("disabled",!0):h.getButton("checklist").hide();config.button_export?h.getButton("export").prop("disabled",!0):h.getButton("export").hide();config.button_mail?h.getButton("mail").prop("disabled",
!0):h.getButton("mail").hide();config.button_print?h.getButton("print").prop("disabled",!0):h.getButton("print").hide();h.setData("ppn",a);h.setData("name","fullview_"+e);h.open()};
...
" textarea").val("");$("#class_"+a+" textarea").val("");$("#toc_"+a+" textarea").val("");$("#year_"+a+" textarea").val("");$("#publisher_"+a+" textarea").val("");$("#isn_"+a+" textarea").val("");$("#series_"+a+" textarea").val("");$("#id_"+a+" textarea").val("");$.focus_first()}},{icon:"glyphicon glyphicon-search",label:" "+configlang.STARTSEARCH,cssClass:"fullview-button-color",action:function(b){var c=b="",c=$.cleanArray($("#title_"+a+" textarea").val().split("\n"));0<c.length&&(b+=" title("+c.join(",")+
")");c=$.cleanArray($("#author_"+a+" textarea").val().split("\n"));0<c.length&&(b+=" author("+c.join(",")+")");c=$.cleanArray($("#subject_"+a+" textarea").val().split("\n"));0<c.length&&(b+=" subject("+c.join(",")+")");c=$.cleanArray($("#class_"+a+" textarea").val().split("\n"));0<c.length&&(b+=" class("+c.join(",")+")");c=$.cleanArray($("#toc_"+a+" textarea").val().split("\n"));0<c.length&&(b+=" toc("+c.join(",")+")");c=$.cleanArray($("#year_"+a+" textarea").val().split("\n"));0<c.length&&(b+=" year("+
c.join(",")+")");c=$.cleanArray($("#publisher_"+a+" textarea").val().split("\n"));0<c.length&&(b+=" publisher("+c.join(",")+")");c=$.cleanArray($("#isn_"+a+" textarea").val().split("\n"));0<c.length&&(b+=" isn("+c.join(",")+")");c=$.cleanArray($("#series_"+a+" textarea").val().split("\n"));0<c.length&&(b+=" series("+c.join(",")+")");c=$.cleanArray($("#id_"+a+" textarea").val().split("\n"));0<c.length&&(b+=" id("+c.join(",")+")");$.when($.new_search("init",b)).then(function(){0==results.hits?$("#assistantcount").html("<h4>"+
configlang.ASSISTANTNO+"</h4>"):$("#assistantcount").html("<h4>"+$.format_hits(results.hits)+" "+configlang.ASSISTANTYES+"</h4>")})}}],onhidden:function(){dialogs.pop()},onshown:function(){dialogs.push(b)}});b.realize();b.getModalHeader().addClass("preview-header");b.setData("name","assistant_"+a);b.open()};
...
$("#passwort").focus(),-1;$.post("/ajax/login",{user:user,pw:pw},function(b){"-1"!=b?(a.close(),$(".nologinarea").addClass("hide"),$(".loginarea").removeClass("hide"),usrconfig=b,timer=!0,$.startTimer(),d="success",c=configlang.LOGINSUCCESS.replace("{firstname}",b.firstname).replace("{lastname}",b.lastname),b.collectable&&(e="warning",f=configlang.COLLECTABLEREMARK)):($.user_message(configlang.LOGINERROR,"warning"),$("#kennung").focus(),usrconfig="");$.focus_first()},"json")}}],onhidden:function(){dialogs.pop();
...
$.cleanArray=function(a){for(var b=0;b<a.length;b++)if(void 0==a[b].trim()||""==a[b].trim())a.splice(b,1),b--;return a};
