...
language:"",layout:"",theme:""},favors={},favormails={},idleTimer,time2warn=1080,time2kill=120,timer=!1,urlport=location.protocol+"//";$.cookie("theme")&&$("[rel='"+$.cookie("theme")+"']").parent().addClass("active");
...
($(".datapoollocal").addClass("hide"),$(".datapoolglobal").removeClass("hide"));""!=searchvals.inittext&&$.when($.new_search("initfacets",searchvals.inittext)).done(function(){console.log("Hallo");searchvals.inittext=""})},"JSON")};
...
$.push_state=function(){if(appstate.pushurl||appstate.first){if(0<=searchvals.text.search(":"))var a=searchvals.text.split(":"),b=a[0],a=a[1];else b="",a=searchvals.text;var c={facetvals:facetvals,statevals:{language:config.language,theme:config.theme,layout:config.layout,ppnlist:statevals.ppnlist}};"*"==a&&(a="{star}");var e=urlport+window.location.host+"/"+encodeURIComponent(a),e=e+(""!=b?"/"+b:"/{}")+("/"+window.btoa(unescape(encodeURIComponent(JSON.stringify(c)))));appstate.first?(window.history.replaceState({text:searchvals.text,
facets:facetvals,states:statevals},a+" | "+xtitle,e),appstate.first=!1):window.history.pushState({text:searchvals.text,facets:facetvals,states:statevals},a+" | "+xtitle,e)}};
...
favormails[a]=$("#mail_"+d).wrap("<p/>").parent().html(),$.user_message(configlang.CHECKLISTSUCCESS,"success"),b.find(".text").text(configlang.CHECKLISTREMOVE),b.find(".fa").removeClass("fa-star"),b.find(".fa").addClass("fa-star-o"));$.update_favors()}}],onhidden:function(){dialogs.pop();statevals.ppnlist.pop();$.push_state()},onshown:function(){dialogs.push(l);statevals.ppnlist.push({ppn:a,ppnlist:b,typ:c,parentppn:e});$.push_state();$.opened_fullview(a,d)}});l.realize();l.getModalHeader().addClass("preview-header");
l.setData("ppn",a);l.setData("name","fullview_"+d);l.open()};
